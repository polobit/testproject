...
public class ZohoAgileMappingImpl implements ZohoAgileMapping {
	...
	/***
	 * saving contact of Account module for company in Agile setting by contact
	 * type is company
	 */
	...
	public void saveAccounts(JSONArray zohoData, Key<DomainUser> key) {
		...
		int counter = 0;
		while (it.hasNext()) {
			HashMap<String, String> dataMap = (HashMap<String, String>) it
					.next();
			...
			if (dataMap.containsKey("Account Name"))
				fields.add(new ContactField(Contact.NAME, dataMap
						.get("Account Name"), null));

			if (dataMap.containsKey("Website"))
				fields.add(new ContactField(Contact.WEBSITE, dataMap
						.get("Website"), "url"));

			if (dataMap.containsKey("Phone"))
				fields.add(new ContactField(Contact.PHONE,
						dataMap.get("Phone"), "main"));

			if (dataMap.containsKey("Mobile"))
				fields.add(new ContactField("Mobile", dataMap.get("Mobile"),
						"home"));

			if (dataMap.containsKey("Fax"))
				fields.add(new ContactField("Fax", dataMap.get("Fax"),
						"home fax"));
			...
			try {
				...
				if (dataMap.containsKey("Shipping Street"))
					shippingAddress.put("Street",
							dataMap.get("Shipping Street"));
				...
				if (dataMap.containsKey("Shipping Country"))
					shippingAddress.put("Country",
							dataMap.get("Shipping Country"));
				if (shippingAddress != null && shippingAddress.length() > 1)
					fields.add(new ContactField(Contact.ADDRESS,
							shippingAddress.toString(), "Home"));
				...
				if (dataMap.containsKey("Billing Country"))
					billingAddress.put("Country",
							dataMap.get("Billing Country"));

				if (billingAddress != null && billingAddress.length() > 1)
					fields.add(new ContactField(Contact.ADDRESS, billingAddress
							.toString(), "Work"));
		...
		BulkActionNotifications.publishconfirmation(
				BulkAction.CONTACTS_IMPORT_MESSAGE, String.valueOf(counter)
	...
	public void saveLeads(JSONArray arrayOfLeads, Key<DomainUser> ownerKey) {
		...
		while (it.hasNext()) {
			...
			if (objectField.containsKey("First Name"))
				contactFields.add(new ContactField(Contact.FIRST_NAME,
						objectField.get("First Name"), null));

			if (objectField.containsKey("Last Name"))
				contactFields.add(new ContactField(Contact.LAST_NAME,
						objectField.get("Last Name"), null));

			if (objectField.containsKey("Email"))
				contactFields.add(new ContactField(Contact.EMAIL, objectField
						.get("Email"), null));

			if (objectField.containsKey("Company"))
				contactFields.add(new ContactField(Contact.COMPANY, objectField
						.get("Company"), null));

			if (objectField.containsKey("Website"))
				contactFields.add(new ContactField(Contact.WEBSITE, objectField
						.get("Website"), "URL"));

			if (objectField.containsKey("Phone"))
				contactFields.add(new ContactField(Contact.PHONE, objectField
						.get("Phone"), "Work"));

			if (objectField.containsKey("Title"))
				contactFields.add(new ContactField(Contact.TITLE, objectField
						.get("Title"), null));

			if (objectField.containsKey("Rating"))
				contactFields.add(new ContactField("star_value", objectField
						.get("Rating"), null));

			if (objectField.containsKey("Mobile"))
				contactFields.add(new ContactField("Mobile", objectField
						.get("Mobile"), "Home"));

			if (objectField.containsKey("Fax"))
				contactFields.add(new ContactField("Fax", objectField
						.get("Fax"), "Home Fax"));

			if (objectField.containsKey("Skype ID"))
				contactFields.add(new ContactField("Skype ID", objectField
						.get("Skype ID"), null));
			...
			try {
				...
				if (address != null && address.length() > 1)
					contactFields.add(new ContactField(Contact.ADDRESS, address
							.toString(), "Work"));
		...
		try {
			for (int i = 0; i < rows.length(); i++) {
				...
				for (int j = 0; j < fields.length(); j++) {
					...
					map = mapper.readValue(fields.getString(j),
	...
	public void saveContact(JSONArray zohoData, Key<DomainUser> key) {
		...
		while (it.hasNext()) {
			Contact ctx = new Contact();
			List<ContactField> contactFields = new ArrayList<ContactField>();
			HashMap<String, String> objectField = (HashMap<String, String>) it
					.next();
	...
	public void saveEvents(JSONArray zohoData, Key<DomainUser> key) {
		...
		while (it.hasNext()) {
			...
			if (dataMap.containsKey("Contact Name")) {
				ctx.type = Type.PERSON;
				fieldContact.add(new ContactField(Contact.FIRST_NAME, dataMap
						.get("Contact Name"), null));
