 ...
 *******************************************************************************/
package com.agilecrm.samples.apps.marketplace;
...
/**
 * Very simple example of using the Google Data APIs with 2-legged OAuth. This
 * fetches the current user's next calendar event and displays it.
 */
public class CalendarServlet extends HttpServlet
{
    ...
    /**
     * Initializes the calendar service using OAuth client key & secret from
     * web.xml. Please note that in production applications the consumer secret
     * should be treated the same as sensitive data that should be protected
     * from casual browsing, as would any password or encryption key.
     * 
     * @param config
     *            Servlet config
    ...
    /**
     * Handles the actual query for the Google Calendar API. This is a simple
     * query that checks the user's default calendar for the next event (limit
     * 1, sort by start time). It also uses 2-legged OAuth (xoauth_requestor_id)
     * to impersonate the user rather than using an individual access token.
     * 
     * @param user
     *            User to request data for
