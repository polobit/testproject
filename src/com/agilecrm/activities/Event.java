...
public class Event
{
	...
	public static List<Event> getEvents(Long start, Long end)
	{
		try
		{
			...
			//return dao.ofy().query(Event.class).filter("search_range >=", start).list();
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	// For search - create a range object for start and end date so that we can search between the two
	@PrePersist
