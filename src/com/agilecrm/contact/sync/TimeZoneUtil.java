 ...
 * href="http://www.w3.org/TR/NOTE-datetime">ISO8601</a>)).
 * <p>
...
public class TimeZoneUtil
{
    ...
    public static TimeZone getTimeZone(String isoFormateDate){
	// parse time zone designator (Z or +00:00 or -00:00)
	// and build time zone id (GMT or GMT+00:00 or GMT-00:00)
	String tzID = "GMT"; // Zulu, i.e. UTC/GMT (default)
	int tzPos = isoFormateDate.indexOf('Z');
	if (tzPos == -1)
	{
	    // not Zulu, try +
	    tzPos = isoFormateDate.indexOf('+');
	    if (tzPos == -1)
	    {
		// not +, try -, but remember it might be used within first
		// 8 charaters for separating year, month and day, yyyy-mm-dd
		tzPos = isoFormateDate.indexOf('-', 8);
	    }
	    if (tzPos == -1)
	    {
		// no time zone specified, assume Zulu
	    ...
	    {
		// offset to UTC specified in the format +00:00/-00:00
		tzID += isoFormateDate.substring(tzPos);
		isoFormateDate = isoFormateDate.substring(0, tzPos);
	    }
	}
	else
	{
    ...
    /**
     * Formats a <code>Calendar</code> value into a ISO8601-compliant date/time
     * string.
     * 
     * @param cal
