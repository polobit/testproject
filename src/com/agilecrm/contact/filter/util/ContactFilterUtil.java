...
public class ContactFilterUtil
{
	...
	if (type == ContactFilter.DefaultFilter.RECENT)
	{
	    ...
	    // current_time
	    return contact_query.filter("created_time < ", current_time).order("-created_time").limit(20).list();
	}
	...
	for (Long contactId : new QueryDocument<Contact>(new ContactDocument().getIndex(), Contact.class).getDocumentIds(filter.rules, null, null))
	{
