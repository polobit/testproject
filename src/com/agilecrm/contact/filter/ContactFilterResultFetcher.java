...
public class ContactFilterResultFetcher {
	...
	public ContactFilterResultFetcher(ContactFilter filter,
			int max_fetch_set_size) {
		this.filter = filter;
		this.max_fetch_set_size = max_fetch_set_size;
	}

	public ContactFilterResultFetcher(String filter_id, int max_fetch_set_size,
		...
		try {
			...
			// if(this.filter != null)
			// UserAccessControlUtil.checkReadAccessAndModifyTextSearchQuery(UserAccessControl.AccessControlClasses.Contact.toString(),
			// filter.rules);
	...
	public ContactFilterResultFetcher(Map<String, Object> searchMap,
			String orderBy, Integer max_fetch_set_size, Integer max_limit) {
	...
	private void setAvailableCount() {
		if (filter != null) {
			...
			if (searchMap.containsKey("type")
					&& Contact.Type.COMPANY.toString().equals(
							searchMap.get("type").toString())) {
		...
		System.out.println("total available contacts : " + number_of_contacts
				+ " , total available companies : " + number_of_companies);
	}
	...
	private DefaultFilter getSystemFilter(String id) {
		...
		if (id.startsWith("#tags/")) {
			...
			try {
				...
				searchMap.put("tagsWithTime.tag",
	...
	private List<Contact> fetchNextSet() {
		...
		if (systemFilter != null) {
			contacts = ContactFilterUtil.getContacts(systemFilter,
					max_fetch_set_size, cursor);
			fetched_count += size();
			setCursor();
			return contacts;
		} else if (searchMap != null) {
			if (orderBy != null)
				contacts = Contact.dao.fetchAllByOrder(max_fetch_set_size,
						cursor, searchMap, true, false, orderBy);
			else
				contacts = Contact.dao.fetchAll(max_fetch_set_size, cursor,
						searchMap);
			fetched_count += size();
			setCursor();
			return contacts;
			...
			try {
				contacts = ContactUtil.getContactsBulk(new JSONArray(
						contact_ids));
				if (contacts.size() == 0) {
					return contacts;
				}
		...
		Collection<Contact> contactCollection = new QueryDocument<Contact>(
				new ContactDocument().getIndex(), Contact.class)
				.advancedSearch(filter.rules, max_fetch_set_size, cursor);
