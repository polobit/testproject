...
public class TaskletAdapter implements Tasklet
{
	...
	// Add to Cron
	public void addToCron(JSONObject campaignJSON, JSONObject subscriberJSON, JSONObject data,
			JSONObject nodeJSON, long timeout, String custom1, String custom2, String custom3) throws Exception
	{
		// Add Log
		log(campaignJSON, subscriberJSON, "Sleeping till " + Util.getCalendarString(timeout));

		// Enqueue Task
		Cron.enqueueTask(campaignJSON, subscriberJSON, data, nodeJSON, timeout, custom1, custom2, custom3);
	}
	...
	public String getStringValue(JSONObject nodeJSON, JSONObject subscriberJSON, JSONObject data, String keyName)
			throws Exception
	{
	...
	public Object getValue(JSONObject nodeJSON, JSONObject subscriberJSON, JSONObject data, String keyName)
			throws Exception
	{
	...
	public String replaceTokens(String value, JSONObject subscriberJSON, JSONObject data) throws Exception
	{
		...
		while (it.hasNext())
		{
			...
			if (value.contains(key))
			{
				...
				System.out.println("Key:" + key + " Token:" + data.getString(key) + " Value:" + value + " " + data);
		...
		while (it2.hasNext())
		{
			...
			{
				System.out.println("Key: *" + key + "* Token: *" + "$subscriber." + key + "* Value:" + value);
				value = value.replace("$subscriber." + key, subscriberDataJSON.getString(key));
				replaced = true;
			}
		}
		...
		if (replaced)
		{
			System.out.println("Replaced " + originalString.replaceAll("\n", " ").replaceAll("\r", " ") + " with "
					+ value.replaceAll("\n", " ").replaceAll("\r", " "));
		}
	...
	// replace
	public String replaceTokensOld(String value, JSONObject data) throws Exception
	{
		boolean replaced = false;
		...
		for (int i = 0; i < tokens.length; i++)
		{
			...
			{
				System.out.println("Converting param " + tokens[i] + " from " + data);
		...
		if (replaced)
			System.out.println("Replaced " + value.replaceAll("\n", " ").replaceAll("\r", " ") + " with "
	...
	public void log(JSONObject campaignJSON, JSONObject subscriberJSON, String message) throws Exception
	{

		System.out.println("Log: " + message.replaceAll("\n", " ").replaceAll("\r", " ") + " " + subscriberJSON + " " + campaignJSON);
