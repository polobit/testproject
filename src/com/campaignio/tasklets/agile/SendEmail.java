...
public class SendEmail extends TaskletAdapter
{
			...
			throws Exception
	{
		...
		if (subscriberJSON.has("isUnsubscribedAll"))
		{
			if (subscriberJSON.getBoolean("isUnsubscribedAll"))
			{
				...
				// Add log
				LogUtil.addLogToSQL(AgileTaskletUtil.getId(campaignJSON), AgileTaskletUtil.getId(subscriberJSON),
						"Email sending skipped for this contact as it is unsubscribed.",
						LogType.UNSUBSCRIBED.toString());
			...
			throws Exception
	{
		...
		if (trackClicks != null && trackClicks.equalsIgnoreCase(TRACK_CLICKS_YES))
		{
			try
			{
				...
				// Get Keyword
				text = convertLinks(text, " ", data, keyword, AgileTaskletUtil.getId(subscriberJSON),
						AgileTaskletUtil.getId(campaignJSON));
				html = convertLinks(html, " ", data, keyword, AgileTaskletUtil.getId(subscriberJSON),
						AgileTaskletUtil.getId(campaignJSON));

				text = convertLinks(text, "\n", data, keyword, AgileTaskletUtil.getId(subscriberJSON),
						AgileTaskletUtil.getId(campaignJSON));
				html = convertLinks(html, "\n", data, keyword, AgileTaskletUtil.getId(subscriberJSON),
						AgileTaskletUtil.getId(campaignJSON));

				text = convertLinks(text, "\r", data, keyword, AgileTaskletUtil.getId(subscriberJSON),
						AgileTaskletUtil.getId(campaignJSON));
				html = convertLinks(html, "\r", data, keyword, AgileTaskletUtil.getId(subscriberJSON),
						AgileTaskletUtil.getId(campaignJSON));
		...
		{
			html = EmailUtil.appendTrackingImage(html, AgileTaskletUtil.getId(campaignJSON),
		...
		LogUtil.addLogToSQL(AgileTaskletUtil.getId(campaignJSON), AgileTaskletUtil.getId(subscriberJSON), "Subject: "
				+ subject, LogType.EMAIL_SENT.toString());
			...
			String campaignId) throws Exception
	{
		...
		for (int i = 0; i < tokens.length; i++)
		{
			...
			if (isSpecialLink(tokens[i]))
			{
				...
				String url = URLShortenerUtil.getShortURL(tokens[i], keyword, subscriberId,
						data.getString(CLICK_TRACKING_ID), campaignId);
	...
	{
		String extensions[] = { ".png", ".jpg", ".jpeg", ".jp2", ".jpx", ".gif", ".tif", ".pbm", ".bmp", ".tiff",
				".ppm", ".pgm", ".pnm", ".dtd" };
		...
		if ((str.toLowerCase().startsWith("http") || str.toLowerCase().startsWith("https"))
				...
				&& !str.toLowerCase().startsWith("http://unscr.be") && !isContains)
			return true;
