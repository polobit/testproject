...
public class SendMessage extends TaskletAdapter
{
			...
			throws Exception
	{
		...
		System.out.println("From " + from + " To: " + to + " Messsage: " + message);
		...
		if (to != null && from != null)
		{
			if (to.startsWith("+92"))
			{
				//Util.sendMessageUsingBulkSMS(Util.changeNumber(to), message, Util.changeNumber(from));
				//Util.sendMessageUsingNowSMS(Util.changeNumber(to), message, Util.changeNumber(from));	
			}
			else
			{
				...
				//Util.sendMessageUsingTwilio(Util.changeNumber(to), message, Util.changeNumber(from));
				// Util.sendMessageUsingTropo(Util.changeNumber(to), message,Util.changeNumber(from));
			}
			
			log(campaignJSON, subscriberJSON, "Send SMS -> From " + from + " To: " + to + " Messsage: " + message);
