...
public class Sex extends TaskletAdapter
{
			...
			throws Exception
	{
		...
		if(!data.has(Rapleaf.RAPLEAF))
		{
			...
			if(rapleafJSON != null)
			{
				Log.addLog(campaignJSON, subscriberJSON, "Rapleaf " + rapleafJSON);
				data.put(Rapleaf.RAPLEAF, rapleafJSON);
			}
			else
				Log.addLog(campaignJSON, subscriberJSON, "Could not retrieve data from Rapleaf");
		}
		...
		if(data.has(Rapleaf.RAPLEAF))
		{
			...
			if(rapLeafJSON.has(Rapleaf.RAPPORTIVE_RESULT) && rapLeafJSON.getString(Rapleaf.RAPPORTIVE_RESULT).equalsIgnoreCase(Rapleaf.RAPPORTIVE_RESULT_SUCCESS))
			{
		...
		// Execute Next Branch
		TaskletManager.executeTasklet(campaignJSON, subscriberJSON, data, nodeJSON, branch);
