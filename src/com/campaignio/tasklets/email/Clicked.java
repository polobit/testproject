...
public class Clicked extends TaskletAdapter
{
			...
			throws Exception
	{
		...
		{
			Cron.enqueueTask(campaignJSON, subscriberJSON, data, nodeJSON, timeout, data.getString(SendEmail.TRACKING_ID), null, null);
		}
		else
		{
			Cron.enqueueTask(campaignJSON, subscriberJSON, data, nodeJSON, timeout, null, null, null);
		}
	}
	...
	{				
		// Log
		log(campaignJSON, subscriberJSON, "Interrupted - we got clicked - " + customData);		

		// Execute Next One in Loop (Yes)
		TaskletManager.executeTasklet(campaignJSON, subscriberJSON, data, nodeJSON, BRANCH_YES);
	}
			...
			throws Exception
	{
		...
		/*// Checking if the link is clicked
		if (data.has(SendEmail.URLS_SHORTENED))
		{

			JSONArray urls = data.getJSONArray(SendEmail.URLS_SHORTENED);
			for (int i = 0; i < urls.length(); i++)
			{
				log(campaignJSON, subscriberJSON, "Checking if email is clicked " + urls.getString(i));
				if (Uti.isURLClicked(urls.getString(i)))
				{
					
					log(campaignJSON, subscriberJSON, "Clicked URL: " + urls.getString(i));
		...
		TaskletManager.executeTasklet(campaignJSON, subscriberJSON, data, nodeJSON, BRANCH_NO);
	}
}
