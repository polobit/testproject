...
class TwitterJob
{
	...
	TwitterJob(String token, String tokenSecret, String status, String subscriberId, String campaignId)
	{
		this.token = token;
	...
	public boolean postStatus()
	{
		try
		{
			...
			Log.addLogFromID(campaign_id, subscriber_id,
					"Twitter - Successfully updated the status to [" + status.getText() + "].");
		...
		{
			Log.addLogFromID(campaign_id, campaign_id, "Tweeting failed " + e.getMessage());
			return false;
		}
	}
...
public class TwitterQueue
{
	...
	// Dao
	private static ObjectifyGenericDao<TwitterQueue> dao = new ObjectifyGenericDao<TwitterQueue>(TwitterQueue.class);
	...
	public static boolean addToTwitterQueue(String account, String token, String tokenSecret, String message,
			String rateLimit, JSONObject subscriberJSON, JSONObject campaignJSON)
	{
		...
		{
			// Get Existing Queue
			TwitterQueue twitterQueue = getTwitterQueueForAccount(account, rateLimit);
	...
	public static TwitterQueue getTwitterQueueForAccount(String account, String rateLimit)
	{
		...
		for(TwitterQueue twitterQueue: twitterQueues)
		{
			try
			{
				...
				if(twitterJobs.size() > 0)
				{
					...
					// Delete the queue for that account if no more jobs are pending
					if(twitterJobs.size() == 0)
						twitterQueue.delete();	
					else
					{
