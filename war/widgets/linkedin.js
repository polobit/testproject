...
$(function()
{
	...
	// Gets Plugin Preferences to check whether to show setup or matching
	// profiles
	...
	$('.linkedin_modify_search').die().live('click', function(e)
	{
		...
		Search_details['plugin_id'] = plugin_id;
...
function showLinkedinMatchingProfiles(plugin_id, data)
{
	...
	if (data.length == 0)
	{
		if (Search_details['keywords'] && Search_details['keywords'] != "")
			linkedinMainError("No matches found for " + "<a href='#search' class='linkedin_modify_search'>" + Search_details['keywords'] + "</a>");
		else
			linkedinMainError("No matches found. " + "<a href='#search' class='linkedin_modify_search'>Modify search</a>");
		return;
	}
...
function showLinkedinProfile(linkedin_id, plugin_id)
{
	...
	$
			.get(
					...
					function(data)
					{
						...
						// Sets the LinkedIn name of the profile to the global
						// variable
						Linkedin_current_profile_user_name = data.name;

						// Sets the LinkedIn connected status to the local
						// variable
						linkedin_connected = data.is_connected;

						// If picture is not available to user then show default
						// picture
						...
						// Gets LinkedIn profile template and populate the
						// fields with details
						...
						if (data.searchResult)
						{
							...
							if ((!data.searchResult.three_current_positions || data.searchResult.three_current_positions.length == 0) && (!data.searchResult.three_past_positions || data.searchResult.three_past_positions.length == 0))
							{
	...
	$('.linkedin_stream').die().live('click', function(e)
	{
		...
		function(data)
		{
			...
			if (data.length == 0)
			{
				...
				// If user have overall updates more than 3, less button is
				// shown
			...
			// Populate the template with update stream details and show in
			// panel
	...
	$('#linkedin_refresh_stream').die().live('click', function(e)
	{
		...
		}).error(function(data)
		{
			...
			if (Stream_Data)
			{
				// Populates the template with the initial update stream on
				// error
...
function sendLinkedInAddRequest(plugin_id, linkedin_id)
{
	...
	$('#send_linked_request').click(function(e)
	{
		...
		// Sends post request to url "core/api/widgets/connect/" to call
		// WidgetsAPI
