var ContactsRouter = Backbone.Router
		.extend({
			...
			contactDetails : function(id, contact) {
				...
				// If hte user refreshes the contacts list view page directly -
				// we should load from the model
				if (!contact)
					if (!this.contactsListView
							|| this.contactsListView.collection.length == 0
							|| this.contactsListView.collection.get(id) == null) {
			...
			editContact : function() {
				...
				var contact = this.contactsListView.collection
						.get(this.contactDetailView.model.id);
				...
				this.opportunityView = new Base_Model_View(
						{
							...
							postRenderCallback : function(el) {
								...
								var json = App_Contacts.contactDetailView.model
										.toJSON();
								var contact_name = json.properties[0].value
										+ " " + json.properties[1].value;
								$('.tags', el)
										.append(
												'<li class="label label-warning"  style="display: inline-block; vertical-align: middle; margin-right:3px;" value="'
														+ json.id
														+ '">'
														+ contact_name
														+ '</li>');
			...
			contactViewAdd : function() {
				var view = new Base_Model_View({
					...
					postRenderCallback : function(el) {
						...
						head.js(LIB_PATH + 'lib/jquery.multi-select.js',
								function() {

									$('#multipleSelect', el).multiSelect();
									$('.ms-selection', el).children('ul')
											.addClass('multiSelect').attr(
													"name", "fields_set")
											.sortable();
								});
					}
			...
			editContactView : function(id) {
				...
				if (!App_Contacts.contactViewListView
						|| App_Contacts.contactViewListView.collection.length == 0
						|| App_Contacts.contactViewListView.collection.get(id) == null) {
					this.navigate("contact-views", {
						trigger : true
					});
				}
				var contact_view_model = App_Contacts.contactViewListView.collection
						.get(id);
			...
			contactFilterEdit : function(id) {
				if (!this.contactFiltersList
						|| this.contactFiltersList.collection.length == 0
						|| this.contactFiltersList.collection.get(id) == null) {
