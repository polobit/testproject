...
var ContactsRouter = Backbone.Router
		.extend({
			...
			"contacts" : "contacts", "contact/:id" : "contactDetails", "import" : "importContacts", "contact-edit" : "editContact",
				...
				/* Views */
				"contact-view-add" : "contactViewAdd", "contact-views" : "contactViews", "contact-custom-view-edit/:id" : "editContactView",

				/* Contact-Filters */
				"contact-filter-add" : "contactFilterAdd", "contact-filter-edit/:id" : "contactFilterEdit", "contact-filters" : "contactfilters",
				"contact-filter/:id" : "showFilterContacts",

				/* New Contact/Company - Full mode */
				// "continue-company": "continueCompany",
				/* Contact bulk actions */
				"bulk-campaigns" : "campaignsBulk", "bulk-tags" : "tagsBulk", "bulk-email" : "emailBulk", "bulk-owner" : "ownerBulk",
				...
				this.opportunityView = new Base_Model_View(
						{
							...
							postRenderCallback : function(el)
							{
								...
								$('.tags', el)
										.append(
												'<li class="tag"  style="display: inline-block; vertical-align: middle; margin-right:3px;" data="' + json.id + '">' + contact_name + '</li>');
			...
			editContactView : function(id)
			{
				...
				if (!App_Contacts.contactViewListView || App_Contacts.contactViewListView.collection.length == 0 || App_Contacts.contactViewListView.collection
						.get(id) == null)
				{
			...
			{
				var searchResultsView = new Base_Collection_View({ url : "core/api/search/" + query, templateKey : "search", individual_tag_name : 'tr',
					cursor : true, data : QUERY_RESULTS, sort_collection : false, page_size : 15, postRenderCallback : function(el)
					{
				 ...
				 * searchResultsView.collection = new
				 * BaseCollection(QUERY_RESULTS, { restKey :
				 * searchResultsView.options.restKey, sortKey :
