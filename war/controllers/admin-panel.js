 ...
 * 
 */
var AdminPanelRouter = Backbone.Router
		.extend({
			...
			get_account_stats_for_domain_from_adminpanel : function(el,
					domainname) {
				console.log("in accountstats object");
				console.log(domainname);
				$
						.ajax({
							url : 'core/api/admin_panel/getdomainstats?d='
									+ domainname,
							type : 'GET',
							success : function(data) {
								console.log(data);
								$(el).find('#account').html(
										getTemplate("domain-info", data));
								var emails = data.emailcount;
								$(el).find('#emailcount').html(
										getTemplate("email-stats", JSON
												.parse(emails)));
			...
			get_customerobject_for_domain_from_adminpanel : function(el,
					domainname) {  var that=this;
			
				$
						.ajax({
							url : 'core/api/admin_panel/getcustomer?d='
									+ domainname,
							type : 'GET',
							success : function(data) {console.log("ssssssssssssssssssssssss");console.log(data);
								$(el).find('#planinfo').html(
										getTemplate("plan-info", data));
			...
			get_collection_of_invoices_for_domain_from_adminpanel : function(
					el, domainname) {
				this.invoicecollection = new Base_Collection_View({
					url : "core/api/admin_panel/getinvoices?d=" + domainname,
					templateKey : "admin-invoice",

					individual_tag_name : 'tr'
				});
				this.invoicecollection.collection.fetch();

				$('.past-invoicecollection', el).html(
						this.invoicecollection.render().el);
			},

			//gets collection of charges of aa paricular customer based on 
			get_collection_of_charges_for_customer_from_adminpanel : function(
					el, customerid) {
				this.chargecollection = new Base_Collection_View({
					url : "core/api/admin_panel/getcharges?d=" + customerid,
				...
				$('.past-chargecollection', el).html(
						this.chargecollection.render().el);
			},
			...
			getDomainUserDetails : function(id) {
				...
				this.usersListViewCollection = new Base_Collection_View(
						{
							url : 'core/api/admin_panel/getParticularDomainUsers?d='
									+ id,
							templateKey : "all-domain",
							individual_tag_name : 'tr',
							postRenderCallback : function(el) {
								head
										.js(LIB_PATH + 'lib/jquery.timeago.js',
												function() {
													$(".last-login-time", el)
															.timeago();
												});
								...
								self
										.get_customerobject_for_domain_from_adminpanel(
												el, domainname);
								self
										.get_account_stats_for_domain_from_adminpanel(
												el, domainname);
							/*	self
										.get_collection_of_invoices_for_domain_from_adminpanel(
												el, domainname);*/
				...
				$("#saveNewPasswordFromAdmin")
						.on(
								...
								function(e) {
									...
									var form_id = $(this).closest('form').attr(
											"id");
									...
									$('#changePasswordForm').find(
											'span.save-status').html(
											getRandomLoadingImg());
												...
												success : function() {
													$('#changePasswordForm')
															.find(
																	'span.save-status')
															.html(
																	"<span style='color:green;margin-left:10px;'>Password changed successfully</span>")
															.fadeOut(5000);
			...
			domainSubscribeDetails : function(id) {
				...
				var subscribe_plan = new Base_Model_View(
						{
							url : "core/api/admin_panel/subscriptionofparticulardomain?d="
									+ id,
							template : "all-domain-admin-subscribe-new",
							window : 'domainSubscribe',
							/*
							 * postRenderCallback : function(el) { // Setup
							 * account statistics set_up_account_stats(el); //
							 * Load date and year for card expiry
							 * card_expiry(el); // Load countries and respective
							 * states head.js(LIB_PATH + 'lib/countries.js',
							 * function() { print_country($("#country", el));
							 * }); },
							 */
							postRenderCallback : function(el) {
								...
								if (!$.isEmptyObject(data)) {
									...
									USER_CREDIRCARD_DETAILS = subscribe_plan.model
											.toJSON().billingData;
									console.log(USER_CREDIRCARD_DETAILS);
									element = setPriceTemplete(
											data.plan.plan_type, el);
								}
								...
								head
										.load(
												CSS_PATH + 'css/jslider.css',
												CSS_PATH
														+ "css/misc/agile-plan-upgrade.css",
												LIB_PATH
														+ 'lib/jquery.slider.min.js',
												function() {
													if ($.isEmptyObject(data))
														setPlan("free");
													else
			...
			purchasePlanFromAdminpanel : function() {
				...
				if (!plan_json.plan) {
					this.navigate("all-domain-users", {
						trigger : true
					});
				...
				var upgrade_plan = new Base_Model_View(
						{
							...
							saveCallback : function(data) {
								window.navigate("domainSubscribe/"
										+ plan.domain_name, {
									trigger : true
								});
								showNotyPopUp(
										"information",
										"You have been upgraded successfully. Please logout and login again for the new changes to apply.",
										"top");
							}
