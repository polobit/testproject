...
function agile_init_handlers() {
			...
			function(e) {
				...
				agile_getContact(email,
						function(val) {
							...
							agile_build_form_template(that,
									"gadget-contact-list", ".contact-list",
									function() {
										$('.saving', el).hide();
										if (val.id == null)
	...
	$(".gadget-show-contact").die()
			.live(
					'click',
					function(e) {
						// Prevent default functionality.
						e.preventDefault();
						// Set context (HTML container where event is
						// triggered).
						var el = $(this).closest(
								"div.gadget-contact-details-tab").find(
								'.show-form');
						// Build show contact form template.
						agile_build_form_template($(this),
								"gadget-contact-summary",
								".show-contact-summary", function() {

									var content = Contacts_Json[$(el).data(
											"content")];
									// Build tags list.
									agile_build_tag_ui($("#added_tags_ul", el),
											content);
	...
	$(".gadget-add-contact").die().live(
			...
			function(e) {
				...
				agile_build_form_template($(this), "gadget-add-contact",
