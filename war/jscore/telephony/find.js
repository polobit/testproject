	...
	$.getJSON("/core/api/contacts/search/phonenumber/" + Sip_Session_Call.getRemoteUri(), function(caller)
	{
		...
		if (caller != null)
		{
			...
			//if (caller.properties[0].name == 'first_name' || caller.properties[1].name == 'last_name')
			{
				...
				if (CALL != undefined)
					CALL.setText(SIP_Call_Noty_IMG+'<span style="margin-top: 10px;display: inline-block;"><i class="icon icon-phone"></i><b>Incoming call  </b>'+ User_Number +'<br>' + User_Name+'<br></span><div class="clearfix"></div>' );
			}
		}
	}).error(function(data)
	{
