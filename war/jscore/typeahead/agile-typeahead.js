						...
						updater : function(items)
						{
							...
							if (isEmailSearch)
							{
								...
								if (email_not_exist && (TYPEHEAD_EMAILS[items] != "No email"))
									$('#' + id, el)
											...
											.find(".tags")
											.append(
													'<li class="tag"  style="display: inline-block;" data="' + TYPEHEAD_EMAILS[items] + '"><a href="#contact/' + TYPEHEAD_TAGS[items] + '">' + items_temp + '</a><a class="close" id="remove_tag">&times</a></li>');
							...
							else
							{
								...
								if (tag_not_exist)
									$('.tags', el)
											.append(
													'<li class="tag"  style="display: inline-block;" data="' + TYPEHEAD_TAGS[items] + '"><a href="#contact/' + TYPEHEAD_TAGS[items] + '">' + items_temp + '</a><a class="close" id="remove_tag">&times</a></li>');
							}
						},
...
function buildcategorizedResultDropdown(items, options)
{
	 ...
	 *  // Sets data-value to name i = $(that.options.item).attr('data-value',
	 * fullname);
