        ...
        for (var d = 0; d < i; d++) {
          for (var h = 0; h < j; h++) {
            if (webrules.tags_time.tags[d] == _agile_contact.tagsWithTime[h].tag) {
              if ((webrules.tags_time.condition == "LAST" && (0 <= (current_time - _agile_contact.tagsWithTime[h].createdTime) && (current_time - _agile_contact.tagsWithTime[h].createdTime) <= (webrules.tags_time.time * 86400000))) || (webrules.tags_time.condition == "BEFORE" && (webrules.tags_time.time >= _agile_contact.tagsWithTime[h].createdTime)) || (webrules.tags_time.condition == "AFTER" && (webrules.tags_time.time <= _agile_contact.tagsWithTime[h].createdTime)) || (webrules.tags_time.condition == "EQUALS" && (webrules.tags_time.time <= _agile_contact.tagsWithTime[h].createdTime && _agile_contact.tagsWithTime[h].createdTime <= (webrules.tags_time.time + 86400000))) || (webrules.tags_time.condition == "BETWEEN" && (webrules.tags_time.time_min <= _agile_contact.tagsWithTime[h].createdTime && _agile_contact.tagsWithTime[h].createdTime <= webrules.tags_time.time_max))) {
                f++;
              }
            }
          }
...
function show_modal(modal_data, modal_options, modal_callback) {
    ...
    SM.show({
      ...
      "contents": function() {
        ...
        else return modal_data.contents;
      },
    });
}
...
function show_noty(noty_data, noty_options, noty_callback) {
  head.js("lib/noty/jquery.noty.js", "/lib/noty/layouts/bottomRight.js", "lib/noty/themes/default.js", function() {
...
function execute_action(modal_data, modal_options, modal_callback, noty_data, noty_options, noty_callback, add_campaign_id, rm_campaign_id, add_score, rm_score, add_tags, rm_tags, email) {
  ...
  if (add_tags.length !== 0 && email) _agile.add_tag(add_tags.toString(), {
    success: function() {
  ...
  if (add_score && email) _agile.add_score(add_score, {
    success: function() {
  ...
  if (rm_score && email) _agile.add_score(rm_score, {
    success: function() {
      console.log("score");
    },
    error: function() {
  ...
  if (rm_tags.length !== 0 && email) _agile.remove_tag(rm_tags.toString(), {
    success: function() {
...
function perform_action(anon_webrules, webrules, modal_data, modal_options, modal_callback, noty_data, noty_options, noty_callback, add_campaign_id, rm_campaign_id, add_score, rm_score, add_tags, rm_tags) {
      ...
      if (email == "null" || email == undefined) {
        if (t == 0) {
          if (len == _counter && len !== 0 && _counter !== 0) {
            execute_action(modal_data, modal_options, modal_callback, noty_data, noty_options, noty_callback, add_campaign_id, rm_campaign_id, add_score, rm_score, add_tags, rm_tags);
          }
        }
      } else if (email) {
        _agile.get_contact(email, {
          success: function(data) {
            ...
            for (var rule in webrules) {
              if (webrules.hasOwnProperty(rule) && rules[rule](webrules, _agile_contact)) {
                counter++;
              }
            }

            if ((t == 0 && len !== 0 && len == _counter) || (t !== 0 && t == counter && len == 0) || (t != 0 && t == counter && len == _counter && len !== 0)) {
              execute_action(modal_data, modal_options, modal_callback, noty_data, noty_options, noty_callback, add_campaign_id, rm_campaign_id, add_score, rm_score, add_tags, rm_tags, email);
            }
          },
          error: function() {
    ...
    _agile.web_rules({
      success: function(data) {
          ...
          for (var s = 0; s < r; s++) {
            ...
            if (_agile_webrules[i].rules[s].LHS == "tags_time" && _agile_webrules[i].rules[s].CONDITION == "EQUALS") {
              webrules.tags_time = {};
            ...
            if (_agile_webrules[i].rules[s].LHS == "title" || _agile_webrules[i].rules[s].LHS == "company" || _agile_webrules[i].rules[s].LHS == "owner_id") {
              var property_json = {};
...
function perform_actions(data, validate)
{
    ...
    for (var i = 0; i < l; i++) {
      ...
      for (var t = 0; t < u; t++) {
        ...
        if (_agile_webrules[i].actions[t].action == "ADD_TAG") add_tags.push(_agile_webrules[i].actions[t].popup_pattern);
        if (_agile_webrules[i].actions[t].action == "REMOVE_TAG") rm_tags.push(_agile_webrules[i].actions[t].popup_pattern);

        // If webrule action is score
        if (_agile_webrules[i].actions[t].action == "ADD_SCORE") add_score = _agile_webrules[i].actions[t].popup_pattern;
        if (_agile_webrules[i].actions[t].action == "SUBTRACT_SCORE") rm_score = _agile_webrules[i].actions[t].popup_pattern;
      }
      ...
      perform_action(anon_webrules, webrules, modal_data, modal_options, modal_callback, noty_data, noty_options, noty_callback, add_campaign_id, rm_campaign_id, add_score, rm_score, add_tags, rm_tags);
    }
