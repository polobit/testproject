...
$(function()
{
	...
	$('#merge-contacts-model').die().live('click', function(event)
	{
		...
		if (!confirm(" Delete the "+ dup_contacts1_array.length+ " duplicate contacts and merge data to master record??"))
			return; 
		}
		else
		   if(!confirm(" Delete the 1 duplicate contact and merge data to master record??"))
			   return; 
		event.preventDefault();
		var checked = false;
		var selected_fields = [];
...
function merge_duplicate_contacts(master_record, properties, selected_fields, custom_fields, remove_fields, websites, emails, phones)
{
	...
	delete_duplicate_contacts('/core/api/bulk/update?action_type=DELETE', dup_contacts1_array, master_record);
}
function delete_duplicate_contacts(url, id_array, master_record)
{
	var json = {};
	json.ids = JSON.stringify(id_array);
	$.ajax({ url : url, type : 'POST', data : json, success : function()
	{
		master_record.save({}, { url : '/core/api/contacts/', success : function()
		{
