/**
 * workflows.js is a script file to deal with common UI Handlers for workflows
 * from client side.
 * 
 * @module Campaigns
 * 
...
$(function()
{
	...
	$('#save-workflow').live('click', function(e)
	{
		...
		// If workflows collection is defined, check for duplicate workflow-name.
		if (App_Workflows.workflow_list_view)
		{
			var workflow_name = $('#workflowform').find('#workflow-name').val();

			// Verifies for duplicate names in the collection, using backbone
			// 'where'
			var is_duplicate_name = App_Workflows.workflow_list_view.collection.where({ name : workflow_name.trim() });

			// If duplicate alert and return.
			if (is_duplicate_name.length > 0)
			{
				alert("Please change the given campaign name as it already exists in campaigns.");
		...
		else
		{
			...
			App_Workflows.workflow_list_view.collection.create(workflow, { success : function()
			{
	...
	$('#delete_campaign_logs').live('click', function(e)
	{
		...
		$.ajax({ url : 'core/api/campaigns/logs/' + campaign_id, type : 'DELETE', success : function()
		{
			location.reload(true);
		} });
	});
								...
								.on(
										...
										function()
										{
											$(this)
													...
													.find('div#workflow-help-detail')
													.html(
															'<h3 style="margin-left:165px">Easy. Peasy.</h3><iframe width="420" height="345" src="//www.youtube.com/embed/0Z-oqK6mWiE?enablejsapi=10&amp;autoplay=1" frameborder="0" allowfullscreen></iframe>');
										});
