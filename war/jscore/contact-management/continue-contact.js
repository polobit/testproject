...
function show_error(modalId, formId, errorClass, htmlText)
{
				...
				if (modal_elem.css('display') !== 'none')
				{
								modal_elem.find('.' + errorClass).html(
																'<div class="alert alert-error" ><a class="close" data-dismiss="alert" href="#">&times</a>' + htmlText + '</div>').show();
				}
				else if (form_elem.css('display') !== 'none')
				{
								form_elem.find('.' + errorClass)
																.html('<div class="alert alert-error" ><a class="close" data-dismiss="alert" href="#">&times</a>' + htmlText + '</div>').show();
				}
}
 ...
 * @param {String}
 *         id within which to search for tags, if ignored tags will be searched
 *         in form_id
...
function serialize_and_save_continue_contact(e, form_id, modal_id, continueContact, is_person, saveBtn, tagsSourceId)
{
				...
				if ($(saveBtn).attr('disabled') || !isValidForm($form)){
					...
					console.log(ele.offset().top - container.offset().top + container.scrollTop());
...
function deserialize_contact(contact, template)
{
				...
				$('.date_input').datepicker({ format : 'mm/dd/yyyy' });
				...
				var fxn_display_company = function(data, item)
				{
								$("#content [name='contact_company_id']")
																.html(
																								'<li class="tag"  style="display: inline-block;" data="' + data + '"><a href="#contact/' + data + '">' + item + '</a><a class="close" id="remove_tag">&times</a></li>');
				...
				if (contact.contact_company_id && contact.contact_company_id.length > 0)
				{
								for (var i = 0; i < contact.properties.length; ++i)
								{
												if (contact.properties[i].name == 'company')
												{
																...
																$("#content [name='contact_company_id']")
																								.html(
																																'<li class="tag"  style="display: inline-block;" data="' + contact.contact_company_id + '"><a href="#contact/' + contact.contact_company_id + '">' + contact.properties[i].value + '</a><a class="close" id="remove_tag">&times</a></li>');
												}
								}
				}
