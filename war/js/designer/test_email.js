...
$("#button_email_html").die().live("click", function(e){
    ...
    if($('#button_email_html').is(':disabled'))
    return;
   
    $('#button_email_html').attr('disabled', 'disabled');
    
    $("#button_email_html").before("<span id='confirmation-text'style='margin: 5px 2px 0px;display: inline-block;text-align: center;float: left;width: 75%; color: red;font-style: italic;'></span>");
    ...
    if(!fromEmailValidator.data("validator").checkValidity()){
    	$("#confirmation-text").text("From_email - Please complete this mandatory field.");
    	 $("#confirmation-text").fadeOut(5000,function(){
			  $(this).remove();
			  $('#button_email_html').removeAttr('disabled', 'disabled');
		  });
    	 $('#button_email_html').removeAttr('disabled', 'disabled');
    	return;
    }
    ...
    $("#confirmation-text").remove();
        ...
        $.ajax({
    		  ...
    		  async:false,
    		  success: function (email) {
    			  $("#button_email_html").before("<span id='confirmation-text'style='margin: 5px 2px 0px;display: inline-block;text-align: center;float: left;width: 75%; color: red;font-style: italic;'>Email has been sent to "+email+"</span>");
    			  $("#confirmation-text").fadeOut(5000,function(){
    				  $("#confirmation-text").remove();
    				  $('#button_email_html').removeAttr('disabled', 'disabled');
    			  });
