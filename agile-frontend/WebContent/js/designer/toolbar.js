    ...
    $.each(First_Nodes_Toolbar_Global, function(index, url) {
        ...
        $.ajaxq("toolbarnodes", {
            ...
            success: function (data) {
                ...
                // Add hiddenid to show node when dropped. Any element can be dropped. A hack to check if it is node.
                ...
                menuNode.attr("title", '<span class="ui-icon ui-icon-lightbulb" style="float:right;"></span>'+data.info);
...
function addDraggingCapability() {
    ...
    $('#designer').droppable({
        ...
        drop: function (event, ui) {
            ...
            //  Check if toolbar was dropped - even a modal dialog can be dragged and dropped. 
            // Suggested right way - add a hidden attribute for nodes and check there
            if ($(ui.draggable).attr("hiddenid") == undefined || $(ui.draggable).attr("hiddenid").length == 0) return;
...
.draggable({             
 ...
 stop: function (event, ui) {
          createCookie("toolbarX", ui.position.left, 100);
