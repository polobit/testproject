...
function addTab(selector, tabName, tabID) {
	...
	var tabLi = "<li id=\"" + tabName + "-li\"><a href=\"#" + tabID + "\">"
			+ tabName + "</a></li>";
...
function getTab(selector, tabName, originalTabName) {
	...
	if ((tabObject == undefined) || (tabObject.length == 0)) {
		// console.log("Adding tab with " + selector.attr("id"), tabName,
		// actualTabID);
...
function generateRadioUI(uiFieldDefinition) {
	...
	$.each(options, function(key, value) {
		...
		var input = "<input type=\"radio\" name=\"" + uiFieldDefinition.name
...
function generateDynamicSelectUI(uiFieldDefinition, url, keyField, valField) {
	...
	var selectContainer = $("<select name='" + uiFieldDefinition.name
			+ "' title='" + uiFieldDefinition.title + "'> " + "</select>");

	if (event && eventHandler)
		selectContainer = $("<select id='" + uiFieldDefinition.id + "' "
				+ getStyleAttribute(uiFieldDefinition.style) + " name='"
				+ uiFieldDefinition.name + "' title='"
				+ uiFieldDefinition.title + "'" + event + "='" + eventHandler
				+ "' type='" + (type == undefined ? 'select' : type)
				+ "'></select>");

	// Fetches data and fill select
	fetchAndFillSelect(url, keyField, valField, appendNameField,
			uiFieldDefinition.options, selectContainer, arrange_type);

	// Opt group
	fetchAndFillSelectOptGroup(url, keyField, valField, appendNameField,
			uiFieldDefinition.optgroup, selectContainer, arrange_type);

	return selectContainer;
}

function fetchAndFillSelectOptGroup(url, keyField, valField, appendNameField,
		optgroup, selectContainer, arrange_type, callback) {

	var selectOptGroupAttributes = "";

	// Populate Options - Naresh 23/04/2014
	if (optgroup !== undefined) {

		$.each(optgroup, function(index, data) {

			selectOptGroupAttributes += "<optgroup label='" + data.label
					+ "'></optgroup>";

			if (data.options !== undefined) {
				// Fetches data and fill select
				fetchAndFillSelect(url, keyField, valField, appendNameField,
						data.options, selectOptGroupAttributes, arrange_type,
						callback);
			}
	...
	$(selectOptGroupAttributes).appendTo(selectContainer);

}

function fetchAndFillSelect(url, keyField, valField, appendNameField, options,
		selectContainer, arrange_type, callback) {
	...
	if (options !== undefined) {
		$.each(options, function(key, value) {
			...
			if (key.indexOf("*") == 0) {
				key = key.substr(1);
				selectOptionAttributes += "<option selected='selected' value='"
						+ value + "'>" + key + "</option>";
			} else
				selectOptionAttributes += "<option value='" + value + "'>"
						+ key + "</option>";
		});
	}
			...
			$.each(array, function(index, json) {
				...
				if (key != undefined && value != undefined) {
					...
					if (key.indexOf("*") == 0) {
						...
						option = "<option selected value='" + value + "'>"
								+ key + "</option>";
					} else
...
function generateSelectUI(uiFieldDefinition, selectEventHandler) {
	...
	$.each(options, function(key, value) {
		if (key.indexOf("*") == 0) {
			key = key.substr(1);
			selectOptionAttributes += "<option selected value='" + value + "'>"
					+ key + "</option>";
		} else
			selectOptionAttributes += "<option value='" + value + "'>" + key
					+ "</option>";
	});

	var optgroup = uiFieldDefinition.optgroup;
	var selectOptGroupAttributes = "";
		...
		$
				.each(
						...
						function(index, data) {
							...
							selectOptGroupAttributes += "<optgroup label='"
									+ data.label + "'>";
								...
								$
										.each(
												...
												function(key, value) {
													if (key.indexOf("*") == 0) {
														key = key.substr(1);
														selectOptGroupAttributes += "<option selected value='"
																+ value
																+ "'>"
																+ key
																+ "</option>";
													} else
														selectOptGroupAttributes += "<option value='"
																+ value
																+ "'>"
																+ key
																+ "</option>";
							...
							selectOptGroupAttributes += "</optgroup>";
	...
	if (selectEventHandler)
		return "<select onchange="
				+ selectEventHandler
				+ "(this,'"
				+ uiFieldDefinition.target_type
				+ "') +  name='"
				+ uiFieldDefinition.name
				+ "' title='"
				+ uiFieldDefinition.title
				+ "' id='"
				+ uiFieldDefinition.id
				+ "'"
				+ (uiFieldDefinition.required ? ("required =" + uiFieldDefinition.required)
						: "") + "> " + selectOptionAttributes
				+ selectOptGroupAttributes + "</select>";
...
function generateMilestonesSelectUI(uiFieldDefinition) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			/*
			 * var array = data["milestones"].split(','); $.each(array,
			 * function( index, milestone ) { if(milestone != undefined ||
			 * milestone != "") { option = "<option value='" + milestone + "'>" +
			 * milestone + "</option>"; // Append to container
			 * $(option).appendTo(selectContainer); } });
			 */
			...
			if (options !== undefined)
				selectContainer = selectDefaultMilestone(true, selectContainer,
						data);
			else
				selectContainer = selectDefaultMilestone(false,
...
function selectDefaultMilestone(isTrue, selectOptionContainer, data) {
	...
	$.each(data, function(index, mile) {
		...
		$.each(mile.milestones.split(","), function(index, milestone) {
			...
			if (isTrue)
				option += '<option value="' + mile.id + '_' + milestone + '">'
						+ mile.name + ' - ' + milestone + '</option>';
			else {// Gets the first track of default milestone
				if (mile.isDefault && index == 0)
					option += '<option value="' + mile.id + '_' + milestone
							+ '" selected>' + mile.name + ' - ' + milestone
							+ '</option>';
				else
					option += '<option value="' + mile.id + '_' + milestone
							+ '">' + mile.name + ' - ' + milestone
...
function generateDefaultUI(uiFieldDefinition) {
	...
	for ( var key in uiFieldDefinition) {
		if (uiFieldDefinition.hasOwnProperty(key)) {
			...
			// style is appended at the end. Refer
			// getStyleAttribute(styleAttributes)
			if (key == "style")
				continue;
	...
	// Add checked
	/*
	 * //This is appended 'checked' for all attributes.( commented by
	 * yasin(13-09-10))
	 * 
	...
	if (tagName == 'textarea') {
		...
		return "<" + tagName + " " + attributes
				+ getStyleAttribute(uiFieldDefinition.style) + "/>";
...
function getStyleAttribute(styleAttributes) {
	...
	var style = " style='";
	$.each(styleAttributes, function(key, value) {
		style += key + ":" + value + ";";
	});
...
function loadTinyMCE(name) {
	...
	var newwindow = window.open('cd_tiny_mce.jsp?id=' + name, 'name',
			strWindowFeatures);
	if (window.focus) {
		newwindow.focus();
	}
...
function load_email_templates(subtype) {
	...
	if (subtype != undefined)
		new_window = window
				.open('templates.jsp?id=tinyMCEhtml_email&t=email&subtype='
						+ subtype, 'name', strWindowFeatures);

	else
		new_window = window.open('templates.jsp?id=tinyMCEhtml_email&t=email',
...
function generateHTMLEditor(uiFieldDefinition, container) {
	...
	htmlDiv += "<textarea  id='tinyMCE" + textAreaName + "' name='"
			+ textAreaName + "' style='width:100%' rows='13' cols='75'>"
			+ value + "</textarea> ";
...
function generatrTemplates(uiFieldDefinition, container) {
	...
	/*
	 * if(uiFieldDefinition.style) htmlDiv += "<textarea id='tinyMCE" +
	 * textAreaName +"' style='width:100%' name='" + textAreaName + "' rows='13'
	 * cols='75'>" + value + "</textarea>"; else
	 */
	$(htmlDiv).appendTo(container);
}
...
function openTwitter() {
	var newwindow = window.open('cd_twitter.jsp', 'twitter',
			'height=700,width=700,location=1');
	if (window.focus) {
		newwindow.focus();
	}
	...
	for ( var i = 0; i < length; i++) {
		...
		if (originalDefinition != undefined)
			tab = getTab(selector, uiFieldDefinition.category,
					originalDefinition["ui"][i].category);
		else
			tab = getTab(selector, uiFieldDefinition.category);
		...
		if (uiFieldType == undefined)
			uiFieldType = uiFieldDefinition.type;
		...
		if (uiFieldType == "select" || uiFieldType == "timezone") {
			...
			uiField = generateSelectUI(uiFieldDefinition,
					uiFieldDefinition.select_event_callback);
		...
		if (uiFieldType == "input"
				&& (uiInputType == "checkbox" || uiInputType == "radio" || uiInputType == "button")) {
			...
			if (uiFieldDefinition.id == "button_email"
					|| uiFieldDefinition.id == "button_email_html")
...
function generateAnchorUI(uiFieldDefinition) {
	...
	if (uiFieldDefinition.event)
		return "<a href='" + href + "'" + " " + style + " "
				+ uiFieldDefinition.event + "='" + uiFieldDefinition.callback
				+ ";return;'>" + uiFieldDefinition.text + "</a>";

	return "<a href='" + href + "'" + " " + style + ">"
			+ uiFieldDefinition.text + "</a>";
}

// Constructs the UI elements from nodeDefinition. IF UI key is elements, it
// constructs tabs based on categories and add elements
// Automatically
...
function constructUI(selector, uiDefinition) {
	...
	// select all desired textarea fields and attach tooltips to
	// them(Yasin(14-09-10))
	...
	// select all desired "select" fields and attach tooltips to
	// them(Yasin(14-09-10))
