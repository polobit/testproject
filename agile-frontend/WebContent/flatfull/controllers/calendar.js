...
function show_model(id)
{
	...
	else
	{
		...
		{
			var template = Handlebars.compile('<li class="tag btn btn-xs btn-default m-r-xs m-b-xs inline-block" data="{{id}}"><a href="#contact/{{id}}" class="v-middle">{{name}}</a><a class="close m-l-xs" id="remove_tag" style="color: #363f44; top: -1px">&times</a></li>');
			var json = {};
