...
var ContactsRouter = Backbone.Router
		.extend({
			...
			dashboard : function()
			{
				...
				/*
				 * var time_int =
				 * parseInt($('meta[name="last-login-time"]').attr('content'));
				 * var time_date = new Date(time_int * 1000);
				 * 
				 * head.js(LIB_PATH + 'lib/jquery.timeago.js', LIB_PATH +
				 * 'jscore/handlebars/handlebars-helpers.js', function() { var
				 * el = $(getTemplate('dashboard1', { time_sec :
				 * (time_date).toString().toLowerCase(), time_format :
				 * time_date.getTime()/1000 })); $("#content").html(el);
				 * 
				 * $("#last-login-time").timeago();
				 * 
				 * setup_dashboard(el); // loadDynamicTimeline("my-timeline",
				 * el); });
				 */
			...
			contacts : function(tag_id, filter_id, grid_view, is_lhs_filter)
			{
				 ...
				 * &&readData('dynamic_contact_filter').indexOf('campaign_status') >=
				 * 0 ) { eraseData('dynamic_contact_filter'); }
				 */
				...
				if ((!readCookie('company_filter') && readCookie('contact_filter_type') != 'COMPANY') && !readCookie("agile_contact_view"))
				{
					if (readData('dynamic_contact_filter'))
					{
						...
						this.customView(readCookie("contact_view"), undefined, 'core/api/filters/filter/dynamic-filter', undefined, is_lhs_filter,
								readData('dynamic_contact_filter'));
						return;
					}
			...
			{
				// Removed previous contact timeline tags from the isotope, if
				// existed
				...
				{
					App_Contacts.customFieldsList = new Base_Collection_View({ url : '/core/api/custom-fields/position', sort_collection : false,
				...
				if (!contact)
				{
					...
					// this.contactsListView.collection.url =
				...
				if (contact.get('type') == 'COMPANY')
				{
					this.contactDetailView = new Base_Model_View({ model : contact, isNew : true, template : "company-detail",
						...
						{
							// contactInnerTabsInvoke(el); hiding the prev,next
							// arrows when viewport suits
							...
							// else if (Twilio.Device.status() == "ready" ||
							// Twilio.Device.status() == "busy")
			...
			sendEmail : function(id, subject, body, cc, bcc)
			{
				...
				if (!canSendEmails(1))
				{
					...
					showModalConfirmation(title, message, "", function(element)
					{
				...
				if (id)
				{
					...
					// For Reply all, id may contains multiple emails. If
					// contains multiple, skip
			...
			customView : function(id, view_data, url, tag_id, is_lhs_filter, postData)
			{
				...
				// if directly called the method, i.e on click of custom view
				// link,
				// the url will be updated if any filter conditions are
				// selected.
				if (readData('dynamic_contact_filter'))
				{
					url = 'core/api/filters/filter/dynamic-filter';
				...
				this.contact_custom_view = new Base_Collection_View(
						{
							...
							postRenderCallback : function(el, collection)
							{
								...
								// To set chats and view when contacts are fetch
								// by
								// infiniscroll
								// setup_tags(el);
								...
								// show list of filters dropdown in contacts
								// list
