...
portlets : function()
{
	head.js(LIB_PATH + 'jscore/handlebars/handlebars-helpers.js', LIB_PATH + 'lib/jquery.gridster.js', function()
	{
...
function addNewPortlet(portlet_type, p_name)
{
	...
	var model;
	var scrollPosition;
	portlet.save(obj, { success : function(data)
	{
		hidePortletsPopup();
		model = data.toJSON();
	 ...
	 * gridster.set_dom_grid_height(); window.scrollTo(0,scrollPosition);
	 * },1000);
	 */
}
...
function deletePortlet(el)
{
	...
	$('#portletDeleteModal > .modal-dialog > .modal-content > .modal-body').html(
			"Are you sure you want to delete Dashlet - " + $('#' + p_id).parent().find('.portlet_header > .portlet_header_name').text().trim() + "?");
}
$('.portlet-delete-modal').live("click", function(e)
{
	...
	/*
	 * Sends Delete request with portlet name as path parameter, and on success
	 * fetches the portlets to reflect the changes is_added, to show add portlet
	 * in the view instead of delete option
	 */
...
$('#portlets-events-model-list > tr').live(
		'click',
		function(e)
		{
			var hrefFlag = false;
			...
			if (!hrefFlag)
			{
				App_Portlets.currentPosition = '' + $(this).parents('.gs-w').find('.column_position').text().trim() + '' + $(this).parents('.gs-w').find(
				...
				$('#update-event-time-1').val(
						(new Date(model.get('start') * 1000).getHours() < 10 ? "0" : "") + new Date(model.get('start') * 1000).getHours() + ":" + (new Date(
								model.get('start') * 1000).getMinutes() < 10 ? "0" : "") + new Date(model.get('start') * 1000).getMinutes());
				$('#update-event-time-2').val(
						(new Date(model.get('end') * 1000).getHours() < 10 ? "0" : "") + new Date(model.get('end') * 1000).getHours() + ":" + (new Date(model
								.get('end') * 1000).getMinutes() < 10 ? "0" : "") + new Date(model.get('end') * 1000).getMinutes());
...
$('#portlets-tasks-model-list > tr').live('click', function(e)
{
	 ...
	 * $("#updateTaskForm")); $("#updateTaskModal").modal('show'); // Fills
	 * owner select element populateUsers("owners-list", $("#updateTaskForm"),
	 * value, 'taskOwner', function(data) {
	 ...
	 * Add notes in task modal showNoteOnForm("updateTaskForm", value.notes);
	 */
	var hrefFlag = false;
	if (e.target.attributes != undefined && e.target.attributes != null && e.target.attributes.length == 0)
		hrefFlag = true;
...
$('.portlets-tasks-select').live('click', function(e)
{
	...
	if ($(this).is(':checked'))
	{
		...
		// templateKey : 'portlets-tasks', individual_tag_name : 'tr' });
...
function addWidgetToGridster(base_model)
{
	...
	if (gridster != undefined)
	{
		...
		if (add_flag)
		{
			gridster.add_widget($('#ui-id-' + base_model.get("column_position") + '-' + base_model.get("row_position")), base_model.get("size_x"), base_model
					.get("size_y"), base_model.get("column_position"), base_model.get("row_position"));
			gridster.set_dom_grid_height();
			window.scrollTo(0,
					((parseInt($('#ui-id-' + base_model.get("column_position") + '-' + base_model.get("row_position")).attr('data-row')) - 1) * 200) + 5);
		}
	}
}
