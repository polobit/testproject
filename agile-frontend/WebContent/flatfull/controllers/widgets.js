...
var WidgetsRouter = Backbone.Router
		.extend({
			...
			"callscript/rules" : "CallScriptShow", "callscript/add-rules" : "CallScriptAdd", "callscript/editrules/:id" : "CallScriptEdit",
				...
				"sync/stripe-import" : "stripe_sync", "sync/shopify" : "shopify", "sync/salesforce" : "salesforce", "sync/zoho-import" : "zoho_sync",
				"sync/quickbook" : "quickbook_import", "sync/xero" : "xero_import", "sync/freshbooks" : "freshbooks_sync",
			...
			addWidget : function()
			{
				...
				$('#prefs-tabs-content').html(LOADING_ON_CURSOR);
				...
				this.Catalog_Widgets_View.collection.fetch({ success : function(data)
				{
			...
			contactSync : function()
			{
				...
				$('#prefs-tabs-content')
						.html(
								...
								'<div class="row prefs-datasync"><div class="col-md-12"><h4 class="m-b">Payment <small>import Contacts from payment gateway</small></h4><div class="row"><div id ="stripe" class="col-md-4 col-sm-6 col-xs-12"></div></div></div></div>' + '<div class="row prefs-datasync"><div class="col-md-12"><h4 class="m-b">Accounting <small>import Contacts from Accounting</small></h4><div class="row"><div id ="freshbook" class="col-md-4 col-sm-6 col-xs-12"></div><div class="col-md-4 col-sm-6 col-xs-12" id ="quickbook"></div></div></div></div>'
				...
				this.freshbooks_sync = new Base_Model_View({ url : 'core/api/freshbooks/import-settings',
			...
			freshbooks_sync : function()
			{
				...
				this.freshbooks_sync_setting = new Base_Model_View({ url : 'core/api/freshbooks/import-settings',
					template : 'admin-settings-import-freshbooks-contacts-form', saveCallback : function(model)
					{
			...
			freshbooks_sync_setting : function()
			{
				...
				$('#PrefsTab .select').removeClass('select');
				$('.contact-sync-tab').addClass('select');
				this.freshbooks_import_settings = new Base_Model_View({ url : 'core/api/freshbooks/import-settings',
					template : 'admin-settings-import-freshbooks-settings', saveCallback : function(model)
					{
			...
			GooglePlus : function(id)
			{
				...
				else
				{
					...
					var widgetDetails = $.parseJSON($.ajax({ url : "core/api/widgets/GooglePlus", async : false, dataType : 'json' }).responseText);
					...
					if (widgetDetails)
					{
						...
						var userData = $.parseJSON($.ajax({ url : "https://www.googleapis.com/plus/v1/people/me?access_token=" + widgetPrefGP['access_token'],
