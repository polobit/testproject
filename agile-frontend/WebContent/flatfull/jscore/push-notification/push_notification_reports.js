 ...
 */
function initPushChartsUI(callback)
{
 ...
 */
function showPushGraphs(campaign_id)
{

	// Daily
	showBar('core/api/push-analytics/notifications/graphreports/' + campaign_id + getOptions() + "&type=date", 'line-daily-chart','daily','Count', null);

	// Hourly
	showBar('core/api/push-analytics/notifications/graphreports/' + campaign_id + getOptions() + "&type=hour", 'line-hourly-chart','hourly','Count', null);

	// Weekly
	showBar('core/api/push-analytics/notifications/graphreports/' + campaign_id + getOptions() + "&type=day", 'line-weekly-chart','weekly', 'Count', null);
}
 ...
 */
function get_push_table_reports(campaign_id)
{

	
	$("#pushReport-table-reports").html(getRandomLoadingImg());

	$.getJSON('core/api/push-analytics/table/reports/' + campaign_id + getOptions(), function(data)
	{

		console.log(data);

		// Load Reports Template
		getTemplate("pushReport-table-reports", data, undefined, function(template_ui){
			if(!template_ui)
				  return;
			//$("#form-reports-all-count").html(data.totalcount);
			$("#pushReport-table-reports").html($(template_ui));	
		}, "#pushReport-table-reports");
