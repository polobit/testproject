   ...
   var el_typeahead =  $('#' + id, el).typeahead({
        ...
        updater: function (items) {
            ...
            if(isEmailSearch)
            {
               ...
               $.each($('#' + id, el).closest("div.controls").find(".tags").children('li'), function (index, tag){
                ...
                if(email_not_exist && (TYPEHEAD_EMAILS[items] != "No email"))
                	$('#' + id, el).closest("div.controls").find(".tags").append('<li class="tag  btn btn-xs btn-primary m-r-xs inline-block"  data="' + TYPEHEAD_EMAILS[items] + '"><a href="#contact/' + TYPEHEAD_TAGS[items] +'">' + items_temp + '</a><a class="close m-l-xs" id="remove_tag">&times</a></li>');
            ...
            else
            {
	            ...
	            // add tag
	            if (tag_not_exist)
	            	$('.tags', el).append('<li class="tag btn btn-xs btn-primary m-r-xs m-b-xs inline-block"  data="' + TYPEHEAD_TAGS[items] + '"><a href="#contact/' + TYPEHEAD_TAGS[items] +'" class="text-white v-middle">' + items_temp + '</a><a class="close m-l-xs" id="remove_tag">&times</a></li>');
            }
        },
