...
$(function(){
		...
		$("#companyModal").on('show.bs.modal', function(data) {
			...
			//	if(!value["custom_data"])  value["custom_data"] = [];
				...
				$('.date_input', $(target)).attr("placeholder","{{agile_lng_translate 'contacts-view' 'select-date'}}");
		...
		$("#personModal").on('shown.bs.modal', function(data){
			...
			if( stat!='none')
			{
				...
				agile_type_ahead("contact_company",$('#personForm'), contacts_typeahead,fxn_display_company,'type=COMPANY','<b>{{agile_lng_translate "others" "no-results"}}</b> <br/> {{agile_lng_translate "others" "add-new-one"}}');
			}
		});
