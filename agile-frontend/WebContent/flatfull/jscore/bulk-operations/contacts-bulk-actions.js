...
function show_bulk_owner_change_page()
	...
	/*	$('#addBulkTags')
		 ...
		 $("#addTagsToContactsBulk").click(function(e)
		{
			...
			if(tag_input && tag_input.length>=0 && !(/^\s*$/).test(tag_input))
			{
				var template = Handlebars.compile('<li class="inline-block tag btn btn-xs m-r-xs m-b-xs btn-default text-xs" data="{{name}}">{{name}}<a class="close" id="remove_tag" style="color: #363f44" tag="{{name}}">&times</a></li>');
