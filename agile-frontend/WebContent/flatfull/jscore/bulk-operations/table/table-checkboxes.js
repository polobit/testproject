    ...
    */ 	
	$('body').on('agile_collection_loaded', function(event, el) {
		...
		if(!$(table_element).hasClass('noDelete')){
			...
			$(table).after('<div><div class="select-none"></div></div><footer class="panel-footer"><a href="#" class="btn btn-danger btn-sm" id="delete-checked"> {{agile_lng_translate "contact-details" "delete"}}</a></footer>');
...
function append_checkboxes(el)
{
	...
	if(SELECT_ALL == true || (Current_Route == 'deals' && SELECT_ALL_DEALS==true) || SUBSCRIBERS_SELECT_ALL == true || SELECT_ALL_TASKS || (Current_Route == "leads" && App_Leads.leadsBulkActions && App_Leads.leadsBulkActions.SELECT_ALL_LEADS == true))
	{
