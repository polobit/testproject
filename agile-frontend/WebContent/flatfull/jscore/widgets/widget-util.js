...
function show_set_up_widget(widget_name, template_id, url, model) {
		...
		if (!App_Widgets.Catalog_Widgets_View
			...
			App_Widgets.Catalog_Widgets_View.collection.fetch({
				success : function() {
					...
					$.getJSON('core/api/widgets/' + widget_name,
							function(data) {
	...
	if (url) {
		...
		if (widget_name == "Shopify" && (model || models[0].attributes.id)) {
			if (model) {
				$('#widget-settings', el).html(getTemplate(template_id, {
					"data" : jQuery.parseJSON(model.prefs)
				}));
			} else if (models[0].attributes.id) {
				$('#widget-settings', el).html(getTemplate(template_id, {
					"data" : jQuery.parseJSON(models[0].attributes.prefs)
				}));
			}
		} else {
			$('#widget-settings', el).html(getTemplate(template_id, {}));
...
function set_up_access(widget_name, template_id, data, url, model) {
		...
		if (!App_Widgets.Catalog_Widgets_View
			...
			App_Widgets.Catalog_Widgets_View.collection.fetch({
				success : function() {
					...
					$.getJSON('core/api/widgets/' + widget_name,
							function(data1) {
								set_up_access(widget_name, template_id, data,
										url, data1)
							});
				}
			});
	...
	$('#widget-settings', el).html(
			getTemplate(widget_name.toLowerCase() + "-revoke-access", json));
...
function saveChargifyWidgetPrefs() {
	...
	save_widget_prefs("Chargify", JSON.stringify(Chargify_prefs),
			function(data) {
