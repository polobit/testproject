...
!function($)
{
	...
	var DateRangePicker = function(element, options, cb)
	{
		...
		this.locale = { applyLabel : 'Apply', clearLabel : "Clear", fromLabel : 'From', toLabel : 'To', weekLabel : 'W', customRangeLabel : 'Custom Range',
			daysOfWeek : Date.CultureInfo.shortestDayNames, monthNames : Date.CultureInfo.monthNames, firstDay : 0 };
		...
		var DRPTemplate = '<div class="daterangepicker dropdown-menu">' + '<div class="calendar left"></div>' + '<div class="calendar right"></div>' + '<div class="ranges">' + '<div class="range_inputs">' + '<div class="daterangepicker_start_input" style="padding-left:11px;">' + '<label for="daterangepicker_start">' + this.locale.fromLabel + '</label>' + '<input class="input-mini form-control" type="text" name="daterangepicker_start" value="" disabled="disabled" />' + '</div>' + '<div class="daterangepicker_end_input" style="padding-left: 11px">' + '<label for="daterangepicker_end">' + this.locale.toLabel + '</label>' + '<input class="input-mini form-control" type="text" name="daterangepicker_end" value="" disabled="disabled" />' + '</div>' + '<span class="m-t-sm inline-block m-l-sm"><button class="btn btn-small btn-primary applyBtn" disabled="disabled">' + this.locale.applyLabel + '</button></span>&nbsp;' + '<span class="m-t-sm inline-block m-l-sm"><button class="btn btn-small btn-default clearBtn">' + this.locale.clearLabel + '</button></span>' + '</div>' + '</div>' + '</div>';
		...
		if (hasOptions)
		{
			...
			if (typeof options.ranges == 'object')
			{
				...
				{
					if (range == "Last 7 Days" || range == "Last Month" || range == "Last Quarter")
					{
		...
		move : function()
		{
			if (this.opens == 'left')
			{
				this.container.css({ top : this.element.offset().top + this.element.outerHeight(),
					right : $(window).width() - this.element.offset().left - this.element.outerWidth(), left : 'auto' });
			}
			else
			{
		...
		renderCalendar : function(calendar, selected, minDate, maxDate)
		{
			...
			for (var row = 0; row < 6; row++)
			{
				...
				for (var col = 0; col < 7; col++)
				{
					...
					html += '<td class="' + cname.replace(/\s+/g, ' ').replace(/^\s?(.*?)\s?$/, '$1') + '" title="' + title + '">' + calendar[row][col]
							.getDate() + '</td>';
				}
				html += '</tr>';
			}
