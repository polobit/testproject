...
//												return value;
																						...
																						//		var html = "<ul class='milestone-value-list tagsinput' style='padding:1px;list-style:none;'>";
																								if (data)
																								{
																												...
																												{				
																																...
																																html += milestones[i]+"</div></td><td class='p-r-none'><div class='m-b-n-xs' style='display:none;'><a class='text-l-none-hover c-p'><i title='Drag' class='icon-move'></i></a><a class='milestone-delete c-p m-l-sm text-l-none'  data-toggle='modal' role='button' href='#'><i title='Delete Milestone' class='task-action icon icon-trash'></i></a></div></td></tr>";
				 ...
				 * 
				 * @param {Object}
				 *         info_json json object containing information about createdtime,
				 *         last logged in time etc..
				 * @param {String}
				 *         date_type specifies the type of date to return (created or logged
				 *         in)
				 */
           ...
           if(val == "yettostart")
				...
				Handlebars.registerHelper('get_normal_activity_type', function(name)
				{
								...
								var name_json = { "DEAL_ADD" : "Deal Created", "DEAL_EDIT" : "Deal Edited", "DEAL_CLOSE" : "Deal Closed", "DEAL_LOST" : "Deal Lost", "DEAL_RELATED_CONTACTS" : " Deal Contacts Changed", "DEAL_OWNER_CHANGE" : "Deal Owner Changed", "DEAL_MILESTONE_CHANGE" : "Deal Milestone Changed","DEAL_ARCHIVE":"Deal Archived","DEAL_RESTORE":"Deal Restored",
				...
				Handlebars
												.registerHelper(
																				...
																				function(value)
																				{
																								...
																								return "<span style='top: -3px' class='text-sm pos-rlt label label-" + type + "'>" + reputation + "</span> <!--<span class='badge badge-" + type + "'>" + value + "</span>-->";
...
/**
	 ...
	 * Plan limits
	 */
	Handlebars.registerHelper('isEmailIntegration', function(options){
		console.log(options);
		
		if(_plan_restrictions.is_email_gateway_allowed())
		{
			return options.fn(this);
		}
		return options.inverse(this);
	...
	Handlebars.registerHelper("getTemplate", function(template_name, json){
		if(!json)
			json = {};
			
		return  new Handlebars.SafeString(getTemplate(template_name, json, "no")); 
			});
	
	Handlebars.registerHelper('aclsWithPlans', function(user, options){
		var json = getDomainUserFromDeserialize(user);
	...
	/*
	 * Returns first occurance of text after "Email subject: " in the given message
	 */
