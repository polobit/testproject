...
//		bigObject['custom_fields'] = get_custom_fields();
	...
	sendEmail : function(id, subject, body, cc, bcc)
	{
		...
		agile_type_ahead("to", el, contacts_typeahead, null, null, "email-search", null, true, true);
		
		agile_type_ahead("email_cc", el, contacts_typeahead, null, null, "email-search", null, true, true);
		
		agile_type_ahead("email_bcc", el, contacts_typeahead, null, null, "email-search", null, true, true);
		...
		if (id)
		{
			...
			if(name && name.length)
			{
				...
				$('#to', el).closest("div.controls").find(".tags").append('<li class="tag" style="display: inline-block;" data="' + data + '"><a href="#contact/' + model.id +'">' + name + '</a><a class="close" id="remove_tag">&times</a></li>');
			}
			else
