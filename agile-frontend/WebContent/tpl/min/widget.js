		...
		{{else}}
			...
			<div style="/*margin: 40px auto;	width: 50%;*/ display:inline;float:left">
				...
				{{#if_equals widget_type "CALL"}}
					<div style="display:inline;float:right;display:none; max-width:135px;" class="text-right {{#is_blank name}}{{getRandomNumber}}{{else}}{{this}}{{/is_blank}}-upgrade" ><p><i>Requires Regular or<br/>Pro plan. <a href="#subscribe-plan">Upgrade</a></i><p></div>
					{{else}}
				 <div style="display:inline;float:right;display:none; max-width:135px" class="text-right {{name}}-upgrade" ><p><i>Please <a href="#subscribe-plan">upgrade</a> <br/> to add more widgets</i><p></div>
				{{/if_equals}}
			{{/if}}
    		{{/if}}
		{{/if_equals}}
    					...
    					<div>
							{{#isAllowedInCurrentPlan "is_custom_widget"}}
								...
								{{else}}
									...
									<div style="display:inline;float:right;display:none; max-width:135px" class="text-right custom-widget-upgrade" ><p><i>{{toSafeString message}}</i><p></div>
