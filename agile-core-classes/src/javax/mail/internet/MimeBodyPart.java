...
public class MimeBodyPart extends BodyPart implements MimePart
{
     ...
     * 
     * If this field is not null, it's return by the {@link #getContent} method.
     * The {@link #getContent} method sets this field if it would return a
     * Multipart or MimeMessage object. This field is is cleared by the
     * {@link #setDataHandler} method.
     * 
     * @since JavaMail 1.5
     */
     ...
     * <p>
     * 
     * If the <code>mail.mime.decodefilename</code> System property is set to
	...
	if (cacheMultipart && (c instanceof Multipart || c instanceof Message) && (content != null || contentStream != null))
	{
	    cachedContent = c;
	    /*
	     * We may abandon the input stream so make sure the MimeMultipart
     ...
     * for the file data. The disposition of this part is set to
     ...
     * Content-Transfer-Encoding to provide the data for this part. If
     * contentType or encoding are null, appropriate values will be chosen. The
     * simple file name is used as the file name for this part and the data in
     * the file is used as the data for this part. The disposition of this part
     * is set to {@link Part#ATTACHMENT Part.ATTACHMENT}.
     * 
     * @param file
     *            the File object to attach
     * @param contentType
     ...
     * @param encoding
     *            the Content-Transfer-Encoding, or null
     * @exception IOException
     *                errors related to accessing the file
     * @exception MessagingException
     *                message related errors
    ...
    /**
     * A FileDataSource class that allows us to specify the Content-Type and
     ...
     * structure, we might need to resync our headers.
     * 
     * <br>
     ...
     * <p>
     * 
     * If the {@link #cachedContent} field is not null (that is, it references a
     * Multipart or Message object), then that object is used to set a new
     * DataHandler, any stream data used to create this object is discarded, and
     * the {@link #cachedContent} field is cleared.
     */
	...
	else
	{
		 ...
		 * 
		 * Override disposition, but attempt to retain existing
	...
	// of tokenizer.
	if (s.equalsIgnoreCase("7bit") || s.equalsIgnoreCase("8bit") || s.equalsIgnoreCase("quoted-printable") || s.equalsIgnoreCase("binary")
    ...
    /**
     * Restrict the encoding to values allowed for the Content-Type of the
     * specified MimePart. Returns either the original encoding or null.
     */
	...
	{
	    /*
	     * multipart and message types aren't allowed to have encodings
	     * except for the three mentioned above. If it's one of these types,
	     * ignore the encoding.
	     */
	...
	try
	{
	    ...
	    /*
	     * If this is a multipart, give sub-parts a chance to update their
	     * headers. Even though the data for this multipart may have come
	     * from a stream, one of the sub-parts may have been updated.
	     */
	    if (cType.match("multipart/*"))
	    {
		...
		if (o instanceof MimeMultipart)
		    ...
		    throw new MessagingException("MIME part of type \"" + type + "\" contains object of type " + o.getClass().getName()
	    ...
	    /*
	     * If the data for this part comes from a stream, we can't update
	     * it.
	     */
	...
	{
	    /*
	     * If the data for this part comes from a stream, just copy it to
	     * the output stream without decoding and reencoding it.
	     */
	    DataHandler dh = part.getDataHandler();
	    if (dh instanceof MimePartDataHandler)
	    {
    ...
    /**
     * A special DataHandler used only as a marker to indicate that the source
     * of the data is a MimePart (that is, a byte array or a stream). This
     * prevents updateHeaders from trying to change the headers for such data.
     * In particular, the original Content-Transfer-Encoding for the data must
     * be preserved. Otherwise the data would need to be decoded and reencoded.
     */
