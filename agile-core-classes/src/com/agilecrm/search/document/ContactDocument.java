...
public class ContactDocument extends com.agilecrm.search.document.Document implements BuilderInterface
{
	...
	public Builder buildDocument(Contact contact)
	{
		...
		try {
			...
			//Trace points
			try{
				System.out.println("contact.id:" + contact.id.toString());
				doc.addField(Field.newBuilder().setName("id_dup").setText(contact.id.toString()));
			}catch(Exception e)
			{
			  System.out.println("Debug errror:" + e.getMessage());	
			}
