	...
	public Builder buildDocument(Contact contact)
	{
		...
		try {
			...
			if (contact.last_contacted > 0L)
			{
				...
				doc.addField(Field.newBuilder().setName("last_contacted__dd__").setNumber(cal.get(Calendar.DATE)));
				doc.addField(Field.newBuilder().setName("last_contacted__mm__").setNumber(cal.get(Calendar.MONTH)+ 1));
				doc.addField(Field.newBuilder().setName("last_contacted__yy__").setNumber(cal.get(Calendar.YEAR)));
			}
