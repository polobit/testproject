...
public class IMAPInboxServlet extends HttpServlet
{
	...
	{
	    IMAPStore store = getStore(serverName, userName, password, port);
	    folders = IMAPInboxUtil.getAllFolders(store);
	}
	catch (Exception e)
	{
	    e.printStackTrace();
	...
	{  if(StringUtils.isNotBlank(search_content)){
	    ...
	    for (IMAPFolder folder : folders)
	    {
	    	...
	    	Message[] items = IMAPInboxUtil.searchMessagesInFolder(folder, searchEmail, searchEmailSubject, count,
		        offset,search_content);
	    	
	    	if(search_content != null && search_content != "" && !search_content.equals(""))
    ...
    public void service(HttpServletRequest req, HttpServletResponse resp) throws IOException
    {
		...
		try{
			...
			if (command.equalsIgnoreCase("oauth_email")){
			    ...
			    String oauthKey = req.getParameter("oauth_key");
			    if(flagset != null && flagset !="" && !flagset.equals("")){
			    	if(StringUtils.equals(flagset, "content")){
			    		emails= getContentUsingOAuth2(host, Integer.parseInt(port), userName, oauthKey, flagset,msgnums,folderNames);
			    		sendResponse(req, resp, new JSONObject().put("emails", emails));
			    	}else{
			    ...
			    if(flagset != null && flagset !="" && !flagset.equals("")){
			    	if(StringUtils.equals(flagset, "content")){
			    		emails= getContentUsingIMAP(host, Integer.parseInt(port), userName, password, flagset,msgnums,folderNames);
			    		sendResponse(req, resp, new JSONObject().put("emails", emails));
			    	}else{
...
//
    ...
    public Boolean setFlag(IMAPStore store, String flag, int[] msgnums, String foldername){
    	try{
    		Folder folder;
    		IMAPFolder sent = null;
    		List<Message> markedMsgList = new ArrayList<Message>();
    		if(!StringUtils.equals(foldername, "Sent") && !StringUtils.equals(foldername, "Draft") && !StringUtils.equals(foldername, "Trash")){
    			folder = store.getFolder(foldername);
    		}else{
    			if(StringUtils.equals(foldername, "Sent")){
    				sent = (IMAPFolder) IMAPInboxUtil.getSentMailFolder(store);
    			}else if(StringUtils.equals(foldername, "Draft")){
    				sent = (IMAPFolder) IMAPInboxUtil.getDraftMailFolder(store);
    			}else if(StringUtils.equals(foldername, "Trash")){
    				sent = (IMAPFolder) IMAPInboxUtil.getTrashMailFolder(store);
    			}
    			folder = store.getFolder(sent.getFullName());
    		}
    		...
    		if(StringUtils.equals("SEEN", flag)){
    			...
    			if(!StringUtils.equals(foldername, "Trash")){
    				Folder folder_trash = (IMAPFolder) IMAPInboxUtil.getTrashMailFolder(store);
    				folder.copyMessages(markedMsgList.toArray(new Message[0]), folder_trash);
    			}
    ...
    public static JSONArray getContentUsingOAuth2(String host, int port, String userEmail, String oauthToken, String flagset,int[] msgnums, String foldername) throws Exception{
    	JSONArray searchResults = new JSONArray();
    	IMAPStore store = OAuth2Authenticator.connectToImap(host, port, userEmail, oauthToken, true);
    	
    	searchResults = IMAPInboxUtil.getMailContent(store,msgnums[0],foldername);
    	
    	return searchResults;
    }
    public JSONArray getContentUsingIMAP(String host, int port, String userEmail, String password, String flagset,int[] msgnums, String foldername) throws Exception{
    	JSONArray searchResults = new JSONArray();
    	IMAPStore store = getStore(host, userEmail, password, port);
    	searchResults = IMAPInboxUtil.getMailContent(store,msgnums[0],foldername);
