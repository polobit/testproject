...
public class SendGridUtil
{
	...
	public static void sendSendGridMails(List<TaskHandle> tasks, EmailSender emailSender)
	{
		try
		{
			...
			for (TaskHandle task : tasks)
			{
						...
						&& htmlArray.toString().length() >= MandrillUtil.MAX_CONTENT_SIZE)
				{
					tempArray.put(new JSONObject().put("to", to).put(SENDGRID_TO_LIST, toArray)
							.put(SENDGRID_SUBJECT_LIST, subjectArray).put(SENDGRID_HTML_LIST, htmlArray).put(SENDGRID_TEXT_LIST, textArray));
			...
			if (toArray.length() != 0)
				tempArray.put(new JSONObject().put("to", to).put(SENDGRID_TO_LIST, toArray).put(SENDGRID_SUBJECT_LIST, subjectArray)
						.put(SENDGRID_HTML_LIST, htmlArray).put(SENDGRID_TEXT_LIST, textArray));
	...
	private static JSONObject getSMTPJSON(JSONObject json, MailDeferredTask deferredtask) throws JSONException
	{
		...
		SMTPJSON.put(UNIQUE_ARGUMENTS,
				new JSONObject().put("domain", deferredtask.domain)
						.put("subject", deferredtask.subject).put("campaign_id", deferredtask.campaignId));
		SMTPJSON.put(
				SUBSTITUTION_TAG,
