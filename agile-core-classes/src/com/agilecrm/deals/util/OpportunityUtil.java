...
public class OpportunityUtil {
	 ...
	 * @return int
	 */
	public static int getDealsCount(Long contactId, String milestone,
			Long ownerId) {
		...
		Map<String, String> fromMilestoneDetails = AgileTaskletUtil
				.getTrackDetails(milestone);

		Map<String, Object> conditionsMap = new HashMap<String, Object>();

		if (contactId != null)
			conditionsMap.put("related_contacts", new Key<Contact>(
		...
		if (milestone != null) {
			conditionsMap.put("milestone", fromMilestoneDetails
					.get("milestone").trim());
	 ...
	 * @return List
	 */
	public static List<Opportunity> getDeals(Long contactId, String milestone,
			Long ownerId) {
		if (contactId == null && milestone == null && ownerId == null)
			return null;
	 ...
	 */
	public static List<Opportunity> getOpportunitiesByFilter(String ownerId,
			String milestone, String contactId, String fieldName, int max,
			String cursor, Long pipelineId) {
		if (pipelineId == null || pipelineId == 0L)
			pipelineId = MilestoneUtil.getMilestones().id;

		return getOpportunitiesByFilterWithoutDefaultPipeLine(ownerId,
				milestone, contactId, fieldName, max, cursor, pipelineId);
	...
	public static JSONObject getOpportunitiesWithMilestones(String ownerId,
			String milestone, String contactId, String fieldName, int max,
			String cursor, Long pipelineId) {
		...
		if (pipelineId == null || pipelineId == 0L
				|| MilestoneUtil.getMilestone(pipelineId) == null)
			pipelineId = MilestoneUtil.getMilestones().id;
		...
		if (!StringUtils.isEmpty(milestone)) {
			...
			if (max != 0)
				dealslist = dao.fetchAllByOrder(max, cursor, conditionsMap,
						true, false, "id");
		...
		// Array of milestones
		Opportunity.MILESTONES = MilestoneUtil.getMilestone(pipelineId).milestones
				.split(",");
		// Iterate through all possible milestones
	 ...
	 * @return Count of total number of milestones.
	 */
	public static int getTotalNumberOfMilestonesByPipeline(Long pipelineId,
			long minTime, long maxTime, String milestone) {
		UserAccessControlUtil
				.checkReadAccessAndModifyQuery("Opportunity", null);
		return dao
				.ofy()
				.query(Opportunity.class)
				.filter("pipeline",
						new Key<Milestone>(Milestone.class, pipelineId))
				.filter("close_date >= ", minTime)
				.filter("close_date <= ", maxTime)
				.filter("milestone", milestone).count();
	}
	 ...
	 * @return Milestone JSONObjects with respect to given time period.
	 */
	public static JSONObject getMilestonesByPipeline(Long pipelineId,
		...
		// Iterate through all possible milestones
		for (String milestone : Opportunity.MILESTONES) {
			int numOpportunities = getTotalNumberOfMilestonesByPipeline(
					pipelineId, minTime, maxTime, milestone.trim());
			milestonesObject.put(milestone, numOpportunities);
		}
	 ...
	 *         times.
	 */
	public static List<Opportunity> getOpportunitiesByPipeline(Long pipelineId,
			long minTime, long maxTime) {
		UserAccessControlUtil
				.checkReadAccessAndModifyQuery("Opportunity", null);
	 ...
	 *         month.
	 */
	public static JSONObject getDealsDetailsByPipeline(Long pipelineId,
			long minTime, long maxTime) {
		// Final JSON Constants
		String TOTAL = "total";
		String PIPELINE = "pipeline";
		...
		// Returns month (key) and total and pipeline
		List<Opportunity> opportunitiesList = getOpportunitiesByPipeline(
				pipelineId, minTime, maxTime);
		for (Opportunity opportunity : opportunitiesList) {
			try {
				// Total and Pipeline (total * probability)
				double total = opportunity.expected_value;
	 ...
	 * @return deals list.
	 */
	public static List<Opportunity> getOpportunitiesByFilter(
			org.json.JSONObject filterJson, int count, String cursor) {
		Map<String, Object> searchMap = new HashMap<String, Object>();
		String sortField = "close_date";
		try {
			if (checkJsonString(filterJson, "pipeline_id")) {
				searchMap.put("pipeline", new Key<Milestone>(Milestone.class,
						Long.parseLong(filterJson.getString("pipeline_id"))));
				if (checkJsonString(filterJson, "milestone"))
					searchMap.put("milestone",
							filterJson.getString("milestone"));
			}

			if (checkJsonString(filterJson, "owner_id"))
				searchMap.put("ownerKey", new Key<DomainUser>(DomainUser.class,
						Long.parseLong(filterJson.getString("owner_id"))));

			if (checkJsonString(filterJson, "archived")) {
				if (!filterJson.getString("archived").equals("all"))
					searchMap.put("archived", Boolean.parseBoolean(filterJson
							.getString("archived")));
			}

			if (checkJsonString(filterJson, "value_filter")
					&& filterJson.getString("value_filter").equalsIgnoreCase(
							"equals")) {
				if (checkJsonString(filterJson, "value")) {
					double value = Double.parseDouble(filterJson
							.getString("value"));
				...
				if (checkJsonString(filterJson, "value_start")) {
					double value = Double.parseDouble(filterJson.getString(
							"value_start").replace("%", ""));
					searchMap.put("expected_value >=", value);
				}
			...
			if (checkJsonString(filterJson, "probability_filter")
					&& filterJson.getString("probability_filter")
							.equalsIgnoreCase("equals")) {
				if (checkJsonString(filterJson, "probability")) {
					long probability = Long.parseLong(filterJson.getString(
							"probability").replace("%", ""));
				...
				if (checkJsonString(filterJson, "probability_start")) {
					long probability = Long.parseLong(filterJson.getString(
							"probability_start").replace("%", ""));
					searchMap.put("probability >=", probability);
				}
				if (checkJsonString(filterJson, "probability_end")) {
					long probability = Long.parseLong(filterJson.getString(
							"probability_end").replace("%", ""));
					searchMap.put("probability <=", probability);
				}
			}
			...
			searchMap
					.putAll(getDateFilterCondition(filterJson, "created_time"));
			...
			if (count != 0)
				return dao.fetchAllByOrder(count, cursor, searchMap, true,
						false, sortField);
	 ...
	 */
	private static Map<String, Object> getDateFilterCondition(
			org.json.JSONObject json, String fieldName) {

		Map<String, Object> searchMap = new HashMap<String, Object>();

		try {
			if (checkJsonString(json, fieldName + "_filter")) {
				if (json.getString(fieldName + "_filter").equalsIgnoreCase(
						"equals")
					...
					if (checkJsonString(json, fieldName + "_start")) {
						long closeDate = Long.parseLong(json
								.getString(fieldName + "_start"));
						searchMap.put(fieldName + " >", closeDate);
					}
					if (checkJsonString(json, fieldName + "_end")) {
						long closeDate = Long.parseLong(json
								.getString(fieldName + "_end"));
						searchMap.put(fieldName + " <", closeDate);
					}
				}
			}
	...
	private static boolean checkJsonString(org.json.JSONObject json, String key) {
		...
		try {
			if (!json.has(key))
				return false;
			if (json.getString(key) == null || json.getString(key) == "null"
					|| json.getString(key).length() == 0)
				return false;
	...
	private static Map<String, Object> getCustomFieldFilters(
			org.json.JSONObject json) {
		Map<String, Object> searchMap = new HashMap<String, Object>();
		List<CustomFieldDef> customFieldList = CustomFieldDefUtil
				.getAllCustomFields(SCOPE.DEAL);
		try {
			for (CustomFieldDef customField : customFieldList) {
				...
				if ((customField.field_type == Type.TEXT
						|| customField.field_type == Type.TEXTAREA || customField.field_type == Type.LIST)
						&& checkJsonString(json, fieldKey)) {
					searchMap
							.put("custom_data.value", json.getString(fieldKey));
				} else if (customField.field_type == Type.CHECKBOX
						&& checkJsonString(json, fieldKey)) {
					if (json.getBoolean(fieldKey))
						searchMap.put(fieldLabel, "on");
				} else if (customField.field_type == Type.DATE
						&& checkJsonString(json, fieldKey)) {
	...
	public static List<Opportunity> getAllOpportunity(Long contactId) {
		...
		conditionsMap.put("related_contacts", new Key<Contact>(Contact.class,
				contactId));
		return dao.listByProperty(conditionsMap);
	}
	...
	public static List<Opportunity> getPendingDealsRelatedToCurrentUser(
		...
		try {
			List<Opportunity> allDealsList = dao
					.ofy()
					.query(Opportunity.class)
					.filter("close_date <=", (new Date()).getTime() / 1000)
			...
			for (Opportunity opportunity : allDealsList) {
				if (!opportunity.milestone.equals("Won")
						&& !opportunity.milestone.equals("Lost"))
	...
	public static List<Opportunity> getPendingDealsRelatedToAllUsers(
		...
		try {
			List<Opportunity> allDealsList = dao.ofy().query(Opportunity.class)
					.filter("close_date <=", (new Date()).getTime() / 1000)
					.filter("archived", false).limit(50).order("close_date")
					.list();
			for (Opportunity opportunity : allDealsList) {
				if (!opportunity.milestone.equals("Won")
						&& !opportunity.milestone.equals("Lost"))
	 ...
	 *         current user.
	 */
	public static Map<Double, Integer> getTotalMilestoneValueAndNumber(
			String milestone, boolean owner, long dueDate, Long ownerId,
			Long trackId) {
		UserAccessControlUtil
				.checkReadAccessAndModifyQuery("Opportunity", null);
		...
		try {
			if (ownerId != null) {
				milestoneList = dao
						...
						.filter("ownerKey",
								new Key<DomainUser>(DomainUser.class, ownerId))
				...
				if (owner)
					milestoneList = dao
							...
							.filter("milestone", milestone)
							.filter("pipeline",
									new Key<Milestone>(Milestone.class, trackId))
							.filter("ownerKey",
									new Key<DomainUser>(DomainUser.class,
											SessionManager.get().getDomainId()))
							.filter("archived", false).list();
		...
		try {
			if (ownerId != null)
				return dao
						.ofy()
						.query(Opportunity.class)
						.filter("milestone", "Won")
						.filter("ownerKey",
								new Key<DomainUser>(DomainUser.class, ownerId))
						.filter("archived", false).list();
			else
				return dao.ofy().query(Opportunity.class)
						.filter("milestone", "Won").filter("archived", false)
						.list();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	 ...
	 * @return list of deals.
	 */
	public static List<Opportunity> getOpportunitiesForBulkActions(String ids,
		...
		try {
			...
			if (idsArray != null && idsArray.length() > 0) {
				...
				for (int i = 0; i < idsArray.length(); i++) {
					dealIds.add(new Key<Opportunity>(Opportunity.class, Long
							.parseLong(idsArray.getString(i))));
				...
				deals = OpportunityUtil.getOpportunitiesByFilter(filterJSON,
						count, null);
				String cursor = deals.get(deals.size() - 1).cursor;
				while (cursor != null) {
					deals.addAll(OpportunityUtil.getOpportunitiesByFilter(
							filterJSON, count, cursor));
	...
	public static void updateSearchDoc(List<Opportunity> deals) {
		...
		AppengineSearch<Opportunity> search = new AppengineSearch<Opportunity>(
				Opportunity.class);
		...
		search.index
				.putAsync(builderObjects.toArray(new Builder[deals.size()]));
	}
	...
	public static List<Opportunity> getWonDealsList(Long minTime, Long maxTime) {
		...
		try {
			ownDealsList = dao.ofy().query(Opportunity.class)
	...
	public static List<Opportunity> getNewDealsList(Long minTime, Long maxTime) {
		...
		try {
			newDealsList = dao.ofy().query(Opportunity.class)
					.filter("created_time >= ", minTime)
	...
	public static List<Opportunity> getOpportunitiesByFilterWithoutDefaultPipeLine(
			String ownerId, String milestone, String contactId,
		...
		try {
			...
			if (StringUtils.isNotBlank(contactId))
				searchMap.put("related_contacts", new Key<Contact>(
						Contact.class, Long.parseLong(contactId)));

			if (!StringUtils.isNotBlank(fieldName))
				fieldName = "-created_time";

			if (max != 0)
				return dao.fetchAllByOrder(max, cursor, searchMap, true, false,
						fieldName);
