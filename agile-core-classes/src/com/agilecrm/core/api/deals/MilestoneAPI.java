...
public class MilestoneAPI
{
    ...
    @GET
    @Path("/pipelinesbyName/{track}")
    @Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
    public List<Milestone> getPipelinesByName(@PathParam("track") String Track)
    {
	List<Milestone> milestone = MilestoneUtil.getMilestonesList(Track);
	return milestone;
    }
}