 ...
 * 
 */
public class VisitorSegmentationQueryGenerator
{
    private static final Logger log = Logger.getLogger(VisitorSegmentationQueryGenerator.class.getName());
    ...
    public VisitorSegmentationQueryGenerator(String domain, String filterJsonString, String startTime, String endTime,
	...
	try
	{
	    segmentationQuery = "select SQL_CALC_FOUND_ROWS email,stats_time from page_visits where domain = '"
		    + domain + "' and email!='' and email!='null' and stats_time between '" + startTime + "' and  '"
		    + endTime + "' ";
		...
		{
		    List<VisitorSegmentationRule> rules = new ArrayList<VisitorSegmentationRule>();
		    
		    JSONObject filterJsonObject = new JSONObject(filterJsonString);
		    JSONArray filters = filterJsonObject.getJSONArray("rules");
		    VisitorSegmentationRule sessionsQuery = null;
		    for (int i = 0; i < filters.length(); i++)
		    {
			VisitorSegmentationRule segmentationRule = new VisitorSegmentationRule();
			String inputType = (String) filters.getJSONObject(i).get("LHS");
			if (inputType.equalsIgnoreCase("Tags"))
			    continue;
			...
			{
			    List<VisitorSegmentationRule> sessionsQueryList = new ArrayList<VisitorSegmentationRule>();
     ...
     * @return
     */
    public String constructQuery(List<VisitorSegmentationRule> rules)
    {
	String buildQuery = " ";
	for (VisitorSegmentationRule rule : rules)
	{
