...
public class UpdateContactsOfDomainDeferredTask implements DeferredTask
{
	...
	try
	{
		...
		for (Contact contact : contacts_list)
		{
		     ...
		     * "Exception while adding contact to text search: "
		     * +contact.id + e); failedIds = failedIds + ", " +
		     * contact.id; }
		     */
		    try
		    {
		...
		catch (Exception e)
		{
			...
			catch (Exception e1)
			{
			    try
			    {
				System.out.println("Exception while adding contact to text search: "
					+ builder.build().getId());
				failedIds = failedIds + ", " + builder.build().getId();
			    }
			    catch (Exception e2)
			    {
				e.printStackTrace();
