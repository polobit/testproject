...
public class SalesforceSync extends OneWaySyncService {
	...
	private String getAccountsFromSalesForce() throws Exception {
		...
		// String query = "SELECT Id, ParentId, Name, Website, Phone, Industry,
		// Description, Type, NumberOfEmployees, BillingStreet, BillingCity,
		// BillingState, BillingCountry, BillingPostalCode FROM Account";
	...
	private void importSalesforceAccounts() throws JSONException, Exception {
		try {
			...
			if (syncStatus.get(ImportStatus.SAVED_ACCOUNTS) > 0)
				BulkActionNotifications.publishconfirmation(BulkAction.COMPANIES_CSV_IMPORT,
						String.valueOf(syncStatus.get(ImportStatus.SAVED_ACCOUNTS)));
	...
	private void importSalesforceContacts() throws Exception {
		try {
			...
			if (syncStatus.get(ImportStatus.SAVED_CONTACTS) > 0)
				BulkActionNotifications.publishconfirmation(BulkAction.CONTACTS_CSV_IMPORT,
						String.valueOf(syncStatus.get(ImportStatus.SAVED_CONTACTS)));
	...
	private void saveContactsInAgile(JSONArray entries) {
		...
		for (int i = 0; i < entries.length(); i++) {
			try {
				...
				// Added condition to mandate emails. It is added here as other
				// sync
				// allows contacts without email
				if (emails == null || emails.size() == 0) {
					// syncStatus.put(ImportStatus.EMAIL_REQUIRED,
					// syncStatus.get(ImportStatus.EMAIL_REQUIRED) + 1);
					// syncStatus.put(ImportStatus.TOTAL_FAILED,
					// syncStatus.get(ImportStatus.TOTAL_FAILED) + 1);
					// continue;
				}
	...
	protected void importSalesforceTasks() {
		...
		try {
			...
			System.out.println("contactIdsMap = " + contactIdsMap.size());

			JSONArray tasksArrayJSON = getTaskIdsFromSalesForce();
			System.out.println(tasksArrayJSON.length());

			saveSalesforceTasksInAgile(tasksArrayJSON);

			if (syncStatus.get(ImportStatus.SAVED_TASKS) > 0)
				BulkActionNotifications.publishconfirmation(BulkAction.TASKS_CSV_IMPORT,
						String.valueOf(syncStatus.get(ImportStatus.SAVED_TASKS)));
		} catch (SocketTimeoutException e) {
			System.out.println("In exception ");
			importSalesforceTasks();
		} catch (Exception e) {
	...
	protected JSONArray getTaskIdsFromSalesForce() throws Exception {
		// String query = "select Id, OwnerId, whoId, whatId, Subject,
		// Description, ActivityDate, Priority, Status, Who.Type, Who.Name,
		// Who.Id From Task";
		// String query = "select Id, whoId, Subject, Description, ActivityDate,
	...
	private void getTasksFromIds(String taskIds) throws Exception {
		taskIds = taskIds.trim();
		if (taskIds.startsWith(","))
			taskIds = taskIds.replaceFirst(",", "");

		System.out.println(taskIds.split(",").length);
		String query = "select Id, whoId, Subject, Description, ActivityDate, Priority, Status From Task where id in ("
				+ taskIds + ")";
		tasksList = new JSONArray(SalesforceUtil.getEntities(prefs, query));
	}
	...
	protected void wrapAccountToAgileSchemaAndSave(JSONObject entry) {
		...
		try {
			...
			if (entry.has("Website"))
				agileCompany.properties
						.add(new ContactField(Contact.WEBSITE, JSONUtil.getJSONValue(entry, "Website"), null));

			if (entry.has("Phone"))
				agileCompany.properties
						.add(new ContactField(Contact.PHONE, JSONUtil.getJSONValue(entry, "Phone"), "main"));

			if (entry.has("Fax"))
				agileCompany.properties
						.add(new ContactField(Contact.PHONE, JSONUtil.getJSONValue(entry, "Fax"), "home fax"));

			if (entry.has("Industry"))
				agileCompany.properties
						.add(new ContactField(Contact.TITLE, JSONUtil.getJSONValue(entry, "Industry"), null));
			...
			// Get notes from Salesforce
			SalesforceImportNoteDeferredTask task = new SalesforceImportNoteDeferredTask(NamespaceManager.get(),
					entry.toString(), SalesforceImportNoteDeferredTask.ACTION_TYPE.ADD_NOTES, agileCompany.id);
	...
	public void sendNotification(String notificationSubject) {
		 ...
		 * syncStatus.get(ImportStatus.SAVED_CONTACTS) == 0 &&
		 * syncStatus.get(ImportStatus.TOTAL_TASKS) == 0) return;
		 */
		...
		if (user != null) {
			...
			SendMail.sendMail("govind@agilecrm.com", notificationSubject + " - " + user.domain, NOTIFICATION_TEMPLATE,
					new Object[] { user, syncStatus }, language);
		}
	}
