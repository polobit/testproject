...
function getSlotDurations()
{
	...
	var initialURL = '/core/api/calendar/webevents/getslotdetails?userid=' + User_Id;
	$
			.getJSON(
					initialURL,
					function(data)
...
function get_slots(s_date, s_slot)
{
	...
	var initialURL = '/core/api/calendar/webevents/getslots?&user_id=' + User_Id + '&date=' + s_date + '&slot_time=' + s_slot + "&timezone_name=" + timezoneName + "&epoch_time=" + epochTime + "&selected_time_epoch=" + selected_epoch + "&agile_user_id=" + Agile_User_Id+"&timezone="+timezone;
...
function save_web_event(formId, confirmBtn)
{
	...
	$
			.ajax({
				url : '/core/api/calendar/webevents/save',
				type : 'PUT',
